...
#define SERV_TCP_PORT 	6789			/* TCP port */
...
#else
extern int	GV_ScConnected;				/* connection made - false */
extern int	GV_SysTimeout;
extern	char	__log_buf[MAXDATA+1];			/* buffer for log messages */
extern	char	__Log_Date[10];				/* time string YYMMDD */	
extern	int  	 __proc_id;				/* current process id */
#endif /* GLOBAL */
...
extern	int	flags;				/* used to receive info */
extern	struct 	hostent *hp;				/* holds IP address of server */
extern	struct	sockaddr_in serv_addr;			/* the server's full address */
#endif /* GLOBAL */
