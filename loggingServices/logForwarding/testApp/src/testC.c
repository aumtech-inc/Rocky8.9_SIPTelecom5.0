...
char	*LogServer;
{
	...
	if ( SfCount <= 0 )
	{
		gaVarLog(mod, 1, "There are no files to be sent.  Exiting.");
		return(0);
	}
	...
	if ( (forward_sw == FORWARD_LOG) && (SfCount > 0) )
	{
		...
		{
			gaVarLog(mod, 0,"%s|%d|Unable to send data (%s) for XFER_FILE to %s. rc=%d",
						__FILE__, __LINE__, XFER_FILE, LogServer,retcode);
			sc_Exit();
			return(retcode);
		}
	...
	if ((rc=sc_SendData(1, "~")) != 0 )// send termination
	{
		gaVarLog(mod, 0,"Failed to send termination signal.");
	}
	else
	{
		gaVarLog(mod, 0,"Successfully sent termination signal (~)");
	}
...
if (strlen(DestFile) > 0)
{
	...
	{
			gaVarLog(mod, 0, "%s|%d|Calling sc_SendFile(%s)",
...
if ((for_fp=fopen(ForwardFile, "r")) == NULL)
	{
	gaVarLog(mod, 0,"Unable to open %s for input. errno=%d",
			ForwardFile, errno);
	return(-1);
	}
...
#ifdef DEBUG
	gaVarLog(mod, 0,"DestFile:<%s>",DestFile);
#endif
