...
void RcvFile(char *encChar)
{
	...
	while (1)
	{
		...
		if (strlen(DestFile) == 1)
		{
			gaVarLog(mod, 1, "%s|%d|Received (%s) termination.",
...
if ((input_fd=fopen(SourceFile, "r")) == NULL)
	{
	gaVarLog(mod, 0,"Unable to open %s. errno=%d.",SourceFile, errno);
	gaVarLog(mod, 0, "%s not appended to %s",SourceFile, DestFile);
	return;
	}

#ifdef DEBUG
	gaVarLog(mod, 0,"log_svc: DestFile:<%s>",DestFile);
#endif

if ((output_fd=fopen(DestFile, "a+")) == NULL)
	{
	gaVarLog(mod, 0,"Unable to open %s. errno=%d.",DestFile, errno);
	gaVarLog(mod, 0,"%s not appended to %s",SourceFile, DestFile);
	return;
	}
...
#ifdef DEBUG
	gaVarLog(mod, 0,"log_svc: <%s>   recvdBytes=%ld",data,recvdBytes);
#endif
...
int decryptFile(char *decFile, char *outFile)
{
    ...
    {
    	gaVarLog(mod, 1, "%s|%d|system command (%s) succeeded.",
