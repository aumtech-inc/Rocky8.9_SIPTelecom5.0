...
int sc_RecvFile(char *destination, int fileOption)
{
	...
	switch(fileOption)
        {
                	...
                	if(access(destination, F_OK) == 0)
                        {
                        	sprintf(__log_buf,
					"WARN: File (%s) exists, "
					"won't overwrite with RF_PROTECT\n",
                                   	destination);
        	...
        	default:
                	sprintf(__log_buf, "Invalid file create option %d. Use "
                                   "RF_APPEND, RF_OVERWRITE or RF_PROTECT. "
	...
	if (sc_RecvData(GV_SysTimeout, -1, data, &recvdataBytes) < 0)
        {
        	sprintf(__log_buf, "Unable to get response from server, "
				   "file=(%s)\n",destination);
        	return (sc_FAILURE);
        }
...
#ifdef DEBUG
	sprintf(__log_buf, "srcfile  = >%s<\n", srcfile);
	Write_Log(ModuleName, __log_buf);
	sprintf(__log_buf, "result   = >%s<\n", result);
	Write_Log(ModuleName, __log_buf);
	sprintf(__log_buf, "response = >%s<\n", response);
	Write_Log(ModuleName, __log_buf);
#endif

	if(srcfile[0] == '\0' || result[0] == '\0' || response[0] == '\0')
        {
        	sprintf(__log_buf, "Invalid response (%s) from server. "
				   "file=(%s)\n", data, destination);
		Write_Log(ModuleName, __log_buf);
        	return (sc_FAILURE);
        }
	...
	{
        	sprintf(__log_buf, "Failed to recv. file (%s), %s\n", 
...
else
{
	...
	while(!done)
	{
		...
		if(recvdBytes < 0)
		{
			sprintf(__log_buf,"Failed to recv. file (%s)\n",
							destination);
			Write_Log(ModuleName, __log_buf);
                       	close(fd);
			unlink(destination);
	...
	{
		sprintf(__log_buf, "stat(%s) failed, errno=%d (%s)\n",
					destination, errno,strerror(errno));
		Write_Log(ModuleName, __log_buf);
		return (sc_FAILURE);
	}	
