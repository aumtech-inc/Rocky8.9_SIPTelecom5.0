...
--------------------------------------------------------------------------*/
int Write_Log(char *mod, char *ptr)
{
	long    theTime;
	FILE    *fp_log;
...
//	sprintf(__log_buf, "%s|%d|Sending size (%s)\n", __FILE__, __LINE__, sizeBuf);
...
#ifdef DEBUG
	sprintf(__log_buf, "%s: Recv'd size >%ld< bytes\n", module, *size);
	Write_Log(module, __log_buf);
#endif
...
while(!done)
	{
        if ( strcmp(whatData, "size") == 0 )
        {
            ...
            {
                sprintf(__log_buf, "Failed to read %s, errno=%d (%s)",
			...
			{
				sprintf(__log_buf, "%s|%d|Read an unexpected character(0x%x). Ignoring.", __FILE__, __LINE__, dataBuf[where]);
				Write_Log(module, __log_buf);
                dataBuf[where] = '\0';
			}
			else
...
if((written = write(sockfd, dataBuf, dataSize)) != dataSize)
	...
	shutdown(sockfd, 2);
	close(sockfd);
	Write_Log(module, "Server disconnected");
	GV_ScConnected = 0;
	return(sc_DISCONNECT);
	}

//   	snprintf(__log_buf, 100, "DJB: %s|%d|wrote (%s)", __FILE__, __LINE__, dataBuf);
//	Write_Log(module, __log_buf);
