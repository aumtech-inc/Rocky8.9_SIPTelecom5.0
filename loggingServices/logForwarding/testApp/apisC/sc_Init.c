...
int sc_Init(char *application, char *host)
{
	...
	for(;;)
	{
		...
		{
			sprintf(__log_buf, 
				"getprotobyname failed, errno=%d (%s)",
		...
		{
			sprintf(__log_buf, "socket failed, errno=%d (%s)",
						errno, strerror(errno));
			Write_Log(ModuleName, __log_buf);
			return (sc_FAILURE);
		}
...
//		Write_Log(ModuleName, __log_buf);
		...
		if (hp == 0)
		{
			sprintf(__log_buf,"Could not obtain address of %s", 
									host);
			Write_Log(ModuleName, __log_buf);
			shutdown(sockfd, 2);
			close(sockfd);
		...
		{
			sprintf(__log_buf, 
			  "Unable to connect to WSS server %s, errno=%d (%s)",
	...
	if(status != 0)
		{
		sprintf(__log_buf,"setsocketopt errno= [%d] , msg = [%s]", 
						errno, strerror(errno));
		Write_Log(ModuleName, __log_buf);
		}
	...
	if(uname(&myhost) == -1)
	{
		sprintf(__log_buf, "uname() failed. errno=%d (%s)",
						errno,strerror(errno));
		Write_Log(ModuleName, __log_buf);
		return(-1);
	}
	...
	do
	{
       	if((nbytes = read(sockfd, &buf[where], 1)) < 0)
		{
			...
			if (errno == ECONNRESET)
			{
				Write_Log(ModuleName, "Connection closed by server\n");
				GV_ScConnected = 0;
				close(sockfd);
				return (sc_FAILURE);
			}
			sprintf(__log_buf, "Failed to recv. response from server, " "errno=%d (%s)", errno, strerror(errno));
			Write_Log(ModuleName, __log_buf);
...
int hostStatus(char *host, int timeout)
{
	...
	if(strcmp(myhost.sysname,  "SCO_SV") == 0)
	{
		...
		{
			sprintf(__log_buf, "popen(%s) failed. errno=%d (%s)",
