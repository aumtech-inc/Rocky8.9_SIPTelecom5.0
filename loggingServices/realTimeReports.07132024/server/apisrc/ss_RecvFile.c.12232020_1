...
int ss_RecvFile(char *destination, int fileOption)
{
	...
	if((fd = fopen(destination, "wb")) == (FILE *)NULL)
	{
		sprintf(__log_buf, "Failed to open file %s for writing, errno=%d (%s)",
	...
	while(done == 0)
		...
		if(recvdBytes < 0)
		{
           	fclose(fd);
			unlink(destination);
			sprintf(__log_buf, "%s|%d|ss_RecvData() failed. rc=%d.", __FILE__, __LINE__, recvdBytes);
			Write_Log(ModuleName, 1, __log_buf);
            return(rc);
		}
       	...
       	else                	/* write to file */
        {
			...
			{
				sprintf(__log_buf, "%s|%d|Failed to write data to file %s, [%d, %s]",
					 __FILE__, __LINE__, destination, errno,strerror(errno)); 
				Write_Log(ModuleName, 0, __log_buf);
				fclose(fd);
               	return (ss_FAILURE);
			}
	...
	{
		sprintf(__log_buf,
			"Failed to send file (%s) receipt confirmation.", destination);
		Write_Log(ModuleName, 0, __log_buf);
		return(rc);
	}
