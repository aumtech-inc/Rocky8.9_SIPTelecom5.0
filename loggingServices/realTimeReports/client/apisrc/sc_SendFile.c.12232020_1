...
int sc_SendFile(char *srcfile)
{
	...
	{
		sprintf(__log_buf, "Cannot access file (%s). [%d, %s]",
	...
	sprintf(__log_buf, "Sending result (%s).", result);
	...
	{
		sprintf(__log_buf, "Failed to open file for read file (%s). [%d, %s]", 
				srcfile, errno, strerror(errno));
       	Write_Log(ModuleName, 0, __log_buf);
       	return (sc_FAILURE);
    }
	...
	while(!done)
	{
		...
		sprintf(__log_buf, "%s|%d|%d=read((%.*s)) from file.", 
	...
	if(rc != sc_SUCCESS)
	{
		sprintf(__log_buf, "Failed to receive confirmation of file (%s) receipt.", srcfile);
		Write_Log(ModuleName, 0, __log_buf);
		return(rc);
	}
	if(atol(data) != fileSize)
	{
		sprintf(__log_buf, "Server received %ld bytes when client sent %ld.",
							atol(data),fileSize);
		Write_Log(ModuleName, 0, __log_buf);
		return(sc_FAILURE);
	}
...
int sc_NewSendFile(char *srcfile)
{
	...
	if(GV_ScConnected == 0)
	{
		Write_Log(yMod, 0, "Server disconnected");
		return(sc_DISCONNECT);
	}
