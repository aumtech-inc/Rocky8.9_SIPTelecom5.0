/*----------------------------------------------------------------------------
File Name	:	
Purpose     :  	Mrcp 2.0 TTS Client Manager.
Author      :	Deep Narsay.
Date        : 	02/10/2004
------------------------------------------------------------------------------*/
...
#include "/home/dan/isp2.2/SIPTelecom3.6/include/TEL_LogMessages.h"
...
int startClient(int index)
{
		...
		if((yTempPid = fork()) == 0)
		{
			...
			//sprintf(yErrMsg, 
			//"Starting ttsClient # %d path<%s>", yCounter, gAppPath); 
			//printf("%s\n", yErrMsg);fflush(stdout);

			myLog(__LINE__, mod, REPORT_VERBOSE, 
				"Starting ttsClient # %s path<%s>", yStrIndex, gAppPath);
			...
			if(yRc < 0)
			{
				...
				myLog(__LINE__, mod, REPORT_NORMAL, "Failed to execl ttsClient. [%d, %s]",
		...
		if(yTempPid < 0)
		{
			myLog(__LINE__, mod, REPORT_NORMAL, 
				"Failed to start ttsClient fork:errno=%d", errno);
...
int main(int argc, char * argv[])
{
	...
	{
		myLog(__LINE__, mod, REPORT_NORMAL, 
	        	"Failed sigaction(SIGCHLD, SA_NOCLDWAIT): errno=%d.", errno);
		sleep(3);
		exit(1);
	}
	...
	{
		myLog(__LINE__, mod, REPORT_NORMAL, 
			"Failed signal(SIGPIPE, SIG_IGN): errno=%d.", errno);
		sleep(3);
		exit(1);
	}
	...
	*/
	...
	myLog(__LINE__, mod, REPORT_VERBOSE, 
				"numResources=%d;  gTotalClients=%d", numResources, gTotalClients);
	...
	{
		myLog(__LINE__, mod, REPORT_NORMAL, 
			"Invalid argument (%d) for total clients to be executed. "
	...
	strcat(gAppPath, "/ttsClient");

	for(yCounter = 0; yCounter < gTotalClients; yCounter++)
	{
		myLog(__LINE__, mod, REPORT_VERBOSE, "Calling startClient counter=%d", gTotalClients);
		startClient(yCounter);
	}
		...
		for(yCounter = 0; yCounter < gTotalClients; yCounter++)
		{
			if(gProcesses[yCounter].pid > 0)
			{
				if(kill(gProcesses[yCounter].pid, 0) == -1)
				{
					...
					{
						myLog(__LINE__, mod, REPORT_NORMAL,
							"Calling starClient counter=%d", gTotalClients);
						startClient(yCounter);
						usleep(500);
					}
				}
...
static int  getNumResources(int *numResources)
{
	...
	{
		myLog(__LINE__, mod, REPORT_VERBOSE, "Environment variable ISPBASE is not found/set.  "
    ...
    *numResources = 0;
    ...
    {
	myLog(__LINE__, mod, REPORT_VERBOSE, 
        	"Can't open file %s. [%d, %s]  Defaulting to 48 ports.",
...
static void myLog(int zLine, char *mod, int mode, char *messageFormat, ...)
{
    ...
    LogARCMsg(mod, mode, "-1", "TTS", "mrcpTTSClient2Mgr", TTS_BASE, message);
}
