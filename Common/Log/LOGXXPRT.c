...
extern int errno;
extern int lm_message(int mode, char *PT_object, int MessageID, char *module, int version, char *format, char *PT_prefix, int m_id, char *msgbuf);
int lm_log_id(char *id);
	...
	case REPORT_STDWRITE:
		{
			...
			switch( MessageID)
			{
			  ...
			  default:
				sprintf(diag_mesg, "REPORT_STDWRITE Unknown Message Format %d", MessageID);
				return( STDWrite( module, diag_mesg));
