/*-----------------------------------------------------------------------------
Program:	isp_monitor
Purpose: 	To monitor processes by watching the transaction process and  
		request and response message queues.
Date:		Unknown
Update:		02/05/96 M. Joshi 
Update:		02/28/96 G. Wenzel added NET_Mnemonics.h
Update:		04/11/96 M. Joshi  added horizontal scrolling
Update:		08/08/97 G. Wenzel changed length of print of Proc_no from 7->10
Update:		11/20/98 D. Barto changed fscanf and output for resmgr
Update:		11/20/98 D. Barto decremented port when statis busy for resmgr
Update:		06/22/99 mpb & gpw updated for use of sendToMonitor.dat.
Update:		06/28/99 gpw removed Select, now just Monitor & Unmonitor
Update:		07/01/99 apj Added resource monitor formatting.
Update:		07/01/99 apj Display correct page number at the bottom of
			     Resource monitor.
Update:		07/01/99 apj Display MORE at the bottom of resource monitor &
			     application monitor on first page if there are 
			     more than 1 pages.
Update:		07/01/99 apj Do not show more on the last page of resource
			     monitor & applications monitor.
Update:		07/01/99 apj If you hit 'n' on last page of resource monitor,
			     go to 1st page.
Update:		07/01/99 apj If you hit 'p' on 1st page of resource monitor,
			     Page number on bottom should read 1.
Update:		07/01/99 apj Put errno and key in error message for
			     Message que.
Update:		07/01/99 apj Changed "View Message Queue Status" to "View
			     Network Services Message Queue Status". 
Update:		07/01/99 apj If you hit 'p' on first page of resource monitor,
			     go to last page.
Update:		07/28/99 gpw chng getch to getch in select_process; avoid hang
Update:		07/29/99 gpw now passes err_msg to load_arc_custom_api();
Update:		08/24/99 gpw added logMsg parm to Status_Resources call
Update:		2000/09/29 sja For Linux: Changed alarm() to timeout()
Update:	2000/10/07 sja 4 Linux: Added function prototypes
Update:	09/08/01 mpb Commented if(option == ERR) exit_program in main_menu 
			routine.
Update:	10/17/02 apj Added GV_AppPid.
Update:	02/18/03 mpb Added Susp for suspended call.
Update:	04/15/03 mpb Added Cot when application is performing COT test.
Update:	10/25/05 mpb Added ROff when application is turned off using resact.
Update: 2009/07/02 djb  Modified to add port # to debug file (for VXI).
-----------------------------------------------------------------------------*/
#include "BNS.h"
#include "resp.h"
#include "ISPSYS.h"
...
static	void view_a_monitored_app();
...
//		printf("%s: %s.\n", err_msg, ModuleName);
	...
	{
		endwin();
		printf("%s: At least 80 columns and 20 lines are needed.\n", 
			ModuleName);
		exit(1);
	}
	MsgLine=LINES-2;
	...
	while ( 1 )
	{
		main_menu_choice = main_menu(menu_screen_title, subtitle);	
		...
		case	1:
			...
			break;
		case	2:	
			display_message_q_status(subtitle);
			break;
		case	3:
...
char	apiName[30];
#ifdef ISP_LOG
	static char mod[]="display_results";
#endif

	ptr = 0;
	get_shmem();
#ifdef ISP_LOG
	sprintf(gMsg, "[%d] tran_tabl_ptr=%p", __LINE__, tran_tabl_ptr);
	LogARCMsg(mod, REPORT_VERBOSE, "3", "MON", "arcmon", 712, gMsg);
#endif 
	if(Mem_ok)
	{
		ptr = tran_tabl_ptr;
...
if (ptr)
{
	...
	if ((LINES - 8) > (no_reco - offset))
		no_lines = no_reco;
	else
		no_lines = offset + LINES - 8;
...
-----------------------------------------------------------------------------*/
void view_a_monitored_app(sr_no, app_pid, monitor_file)
int sr_no;
int app_pid;
char *monitor_file;
{
...
gpw 6/22/99
----------------------------------------------------------------------------*/
void display_cur_page(filename)
char *filename;
{
...
void display_resource_page(char *parmResFile)
{
	int line_cnt=0;			/* line counter */
	int j;				/* loop variable */
	int page_cnt=0;			/* to count number of pages */
...
----------------------------------------------------------------------------*/
void display_page_heading(selected_app, page_no, app_pid)
int selected_app;	
int page_no;	
int app_pid;	
{
...
char	port_str[20];
#ifdef ISP_LOG
	static char mod[]="find_tran_proc_pid";
#endif
	...
	*app_pid=0;
	...
	if((tran_shmid = shmget(Shm_key, SIZE_SCHD_MEM, 0))<0)
	{
		...
		*app_pid = -1;
#ifdef ISP_LOG
	sprintf(gMsg, "[%d] (%s) Services not running", __LINE__, ArcServiceName);
	LogARCMsg(mod, REPORT_VERBOSE, "3", "MON", "arcmon", 712, gMsg);
#endif 
		return (-1);
	}
...
#ifdef ISP_LOG
	sprintf(gMsg, "[%d] tran_tabl_ptr=%p tran_shmid=%d", __LINE__, tran_tabl_ptr, tran_shmid);
	LogARCMsg(mod, REPORT_VERBOSE, "3", "MON", "arcmon", 712, gMsg);
#endif 
...
-----------------------------------------------------------------------------*/
int recv_msg(int q_id, Msg *message, long pid)
{
	int	ret;
...
static int get_service_info(char *prog_name, int *service, key_t *shm_key, long *que_no, char *service_name, char *title)
{
	...
	for(i=0; i< (int)strlen(prog_name); i++)
	{
		if (islower(prog_name[i]))
		{
			prog_name[i] = toupper(prog_name[i]);
		}
	}

	if (strstr(prog_name, TELECOM_STR) != NULL)
	{
		*service = TEL;
	...
	else if (strstr(prog_name, SNA_STR) != NULL)
	{
		*service = SNA;
		sprintf(service_name, "%s", "SNA");
		*shm_key = SHMKEY_SNA;
		*que_no  = SNA_REQS_MQUE;
		sprintf(title, "%s", "SNA Server Monitor");
	}
	else if (strstr(prog_name, TCP_STR) != NULL)
	{
		*service = TCP;
		sprintf(service_name, "%s", "TCP");
		*shm_key = SHMKEY_TCP;
		*que_no  = TCP_REQS_MQUE;
		sprintf(title, "%s", "TCP Server Monitor");
	}
	else if (strstr(prog_name, WSS_STR) != NULL)
	{
		*service = WSS;
		sprintf(service_name, "%s", "WSS");
		*shm_key = SHMKEY_WSS;
		*que_no  = WSS_RESP_MQUE;
		sprintf(title, "%s", "WSS Server Monitor");
	}
else
	{
		fprintf(stderr, 
		"%s: Service type could not be determined from <%s>\n",
		 ModuleName, prog_name);
		return(-1);
	}
}
...
/* gpw don't need 		display(LINES-2,1,CLEAR,""); */
			...
			sprintf(monitor_file,"%s/monitor.%-d.%d", 
						Monitor_dir, app_pid, port_no);	
			if ( access(monitor_file, F_OK) != 0 )
			{
				sprintf(monitor_file,"%s/monitor.%-d", Monitor_dir, app_pid);	
			}
