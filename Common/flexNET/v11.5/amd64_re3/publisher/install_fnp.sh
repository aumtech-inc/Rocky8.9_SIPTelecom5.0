...
case `uname` in
    "Darwin")
        SERVICE_DEST="/Library/Application Support/FLEXnet Publisher/Service/11.5.0"
        ;;
    "Linux")
        if file ${service} | grep "ELF 64" > /dev/null
        then
            SERVICE_DEST="/usr/local/share/macrovision/service64/11.5.0"
        else
            SERVICE_DEST="/usr/local/share/macrovision/service/11.5.0"
        fi
        ;;
esac
