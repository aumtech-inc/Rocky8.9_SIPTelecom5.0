...
extern int get_mac_address(char *uname, char *mac_addr, char *msg);
...
static int sGetOS(char *zOS);
static int sGetInterface(char *zInterface);
...
#define SLEEP_TIME 3
	...
	if ((rc = sGetInterface(interface)) < 0 )
	{
		exit(0);
	}
	...
	count=0;
	if ((rc = get_mac_address(interface, mac_addr, msg)) != 0 )
	{
...
static int sGetProcessor(char *zProcessor, char *zErrorMsg)
{
    ...
    {
		fprintf(stderr, 
			"lscpu command failed to obtain CPU info [%d: %s].  "
			"Defaulting to Xeon.",
...
static int sGetInterface(char *zInterface)
{
	...
	sprintf (zInterface, "%s", "eth0");

	if((fp = popen("nmcli connection show --active |grep -v \"NAME\" | wc -l", "r")) == NULL)
    {
        fprintf(stderr,
			"nmcli command failed to retrieve number of network devices.  Defaulting to (%s).", zInterface);
        return(0);
    }
    ...
    {
        fprintf(stderr,
			"[%d] nmcli command failed to retrieve number of network devices.  Defaulting to (%s).",
			__LINE__, zInterface);
		pclose(fp);
        return(0);
    }
	...
	if ( atoi(buf) == 2 ) 
	{
	    if((fp = popen("nmcli connection show --active |egrep -v \"NAME|^lo\"  |cut -d' ' -f1", "r")) == NULL)
	    {
	        fprintf(stderr,
	            "[%d] nmcli command failed to retrieve the network interface.  Defaulting to (%s).", __LINE__, zInterface);
			pclose(fp);
	        return(0);
	    }
		...
		if ( strlen(buf) > 0 )
		{
			sprintf(zInterface, "%s", buf);
			return(0);
		}
		fprintf(stderr,
	            "[%d] nmcli command failed to retrieve the network interface.  Defaulting to (%s).",
				__LINE__, zInterface);
		return(0);
	}
	if ( atoi(buf) < 0 ) 
...
static int sGetOS(char *zOS)
{
    ...
    {
        fprintf(stderr,
			"[%d] Failed to read OS from (%s).  Defaulting to (9.2).", __LINE__, file);
		sprintf(zOS, "%s", "9.2");
        return(0);
    }
