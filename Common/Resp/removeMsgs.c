/*---------------------------------------------------------------------------
Purpose of this program is to remove the messages from the message Que.
It removes only one message at a time and shows it on standard out.
...
if(argc != 2)
{
	fprintf(stdout,"Usage:%s TCP,TEL,SNA,DYNMGR or messgeQueID\n", argv[0]);
	exit(0);
}
...
fprintf(stderr, "About to remove messages for <%s>, que #<%x, %o>, que id %d\n", argv[1],dyn_mgr_que, dyn_mgr_que, dyn_mqid);
mesg2.mesg_type2 = 0;		/* message of dynamic manager type */
