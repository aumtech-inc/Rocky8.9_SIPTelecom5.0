...
static void output_to_stderr(const char *label, va_list ap, char *file,
   ...
   va_copy(ap_copy, ap);
   vsprintf(message2, format, ap_copy);
   va_end(ap_copy);
	if ( strncmp(message2, "ERROR", 5) == 0 )		// BT-167
	{
		logMode = 1;		// REPORT_NORMAL
	}
	else if ( strncmp(message2, "WARNING", 7) == 0 )
	{
		logMode = 128;		// REPORT_DETAIL
	}
	else
	{
		logMode = 2;		// REPORT_VERBOSE
	}
	
   LogARCMsg("prxy", logMode, "-1", "", "arcsiproxd", -1, message2);
