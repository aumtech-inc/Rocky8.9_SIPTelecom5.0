...
*/
   ...
   if ((contact == NULL) ||
      ...
      sts = sip_find_direction(ticket, NULL, 0, URLMAP_SIZE);
      if (sts != STS_SUCCESS) {
   ...
   */
   ...
   if (expires > 0)
   {
      ...
      DEBUGC(DBCLASS_REG,"[%s, %d] expires=%d url1_contact->host=(%s)  url1_to->host=(%s)",
      ...
      for (i=0; i<URLMAP_SIZE; i++)
      {
         ...
         {
//              DEBUGC(DBCLASS_REG,"[%s, %d] DJB: urlmap[%d].active=%d NOT ACTIVE",
//                  __FILE__, __LINE__, i, urlmap[i].active);
            if (j < 0) /* remember first hole */
			{
				j=i;
			}
            continue;
         ...
         {
            DEBUGC(DBCLASS_REG,"[%s, %d] urlmap[%d].active=%d (%s) IS ACTIVE",
                    __FILE__, __LINE__, i, urlmap[i].active,
         ...
         if (compare_url(url1_contact, url2_contact)== STS_SUCCESS)  {

			int tmp_port1 = (url1_contact->port == NULL)?5060:atoi(url1_contact->port);
			int tmp_port2 = (url2_contact->port == NULL)?5060:atoi(url2_contact->port);

			//DDN_ADDED: both ports are specified but are different
			if(tmp_port1 == tmp_port2) {
				
             DEBUGC(DBCLASS_REG, "found entry for %s@%s:%d <-> %s@%s:%d at "
                   "slot=%i, exp=%li",
                   (url1_contact->username) ? url1_contact->username : "*NULL*",
                   (url1_contact->host) ? url1_contact->host : "*NULL*",
					tmp_port1,
                   (url2_contact->username) ? url2_contact->username : "*NULL*",
                   (url2_contact->host) ? url2_contact->host : "*NULL*",
					tmp_port2,
                   i, (long)urlmap[i].expires-time_now);
             break;
			}

             DEBUGC(DBCLASS_REG, "DDN: Ports not matching for %s@%s:%d <-> %s@%s:%d at "
                   "slot=%i, exp=%li",
                   (url1_contact->username) ? url1_contact->username : "*NULL*",
                   (url1_contact->host) ? url1_contact->host : "*NULL*",
					tmp_port1,
...
//                i);
		...
		DEBUGC(DBCLASS_REG,"[%s, %d] Create new entry for %s@%s at slot=%i",
			__FILE__, __LINE__,
			(url1_contact->username) ? url1_contact->username : "*NULL*",
			(url1_contact->host) ? url1_contact->host : "*NULL*", i);
   ...
   { /* expires > 0 */
         ...
         if (compare_url(url1_contact, url2_contact)==STS_SUCCESS)
         {
			...
			if(tmp_port1 == tmp_port2)
			{
            	DEBUGC(DBCLASS_REG, "removing registration for %s@%s at slot=%i",
                   (url2_contact->username) ? url2_contact->username : "*NULL*",
                   (url2_contact->host) ? url2_contact->host : "*NULL*", i);
            	urlmap[i].expires=0;
            	break;
			}
         }
    ...
    for (i=0; i<URLMAP_SIZE; i++)
    {
        ...
        {
            DEBUGC(DBCLASS_REG,"[%s, %d] urlmap[%d].active/reghost/truehost/masqhost=[%d, %s, %s, %s]",
                __FILE__, __LINE__, i, urlmap[i].active,
                (urlmap[i].reg_url->host) ? urlmap[i].reg_url->host : "*NULL*",
                (urlmap[i].true_url->host) ? urlmap[i].true_url->host : "*NULL*",
