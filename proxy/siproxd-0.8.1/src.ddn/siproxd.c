...
struct call_id_map_s call_id_map[CALL_ID_MAP_SIZE];
int call_id_map_idx = 0;
int invite_only_pointer = 0;
 ...
 */
   ...
   memset (call_id_map,    0, sizeof(call_id_map));
   log_set_pattern(0);
 ...
 *****************************/
   while (!exit_program) {
      ...
      ticket.direction=0;
      buff[buflen]='\0';
      DEBUGC(DBCLASS_BABBLE,"received %zd bytes of data, \n%s\n", buflen, buff);
	  ...
	  if(MSG_IS_OPTIONS(ticket.sipmsg) &&
          MSG_IS_REQUEST(ticket.sipmsg)) {

           sip_gen_response(&ticket, 200 );
	  }
      else if (MSG_IS_REGISTER(ticket.sipmsg) && 
          MSG_IS_REQUEST(ticket.sipmsg)) {
