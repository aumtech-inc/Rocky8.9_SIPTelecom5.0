...
extern struct call_id_map_s call_id_map[];
extern int    call_id_map_idx;
extern int    invite_only_pointer;
 ...
 */
int  sip_find_direction(sip_ticket_t *ticket, int *urlidx, int beg_idx, int end_idx) {
   int type;
   int i, sts;
   ...
   int is_it_invite   = (MSG_IS_INVITE(ticket->sipmsg))?1:0;
   int is_it_bye 	  = (MSG_IS_BYE(ticket->sipmsg))?1:0;
   int is_it_ack 	  = (MSG_IS_ACK(ticket->sipmsg))?1:0;
   int is_it_notify   = (MSG_IS_NOTIFY(ticket->sipmsg))?1:0;

	int bye_idx = 0;
	int call_id_found = 0;

#if 0
   if(urlidx && is_it_invite){
		invite_only_pointer++;
   ...
   for (i=0; i<URLMAP_SIZE; i++) {
      if (urlmap[i].active == 0) {
        //DEBUGC(DBCLASS_SIP, "%s %d sip_find_direction: DDNDEBUG: continue, urlmap[%d] is not active", __FILE__, __LINE__, i);
		continue;
      }
      ...
      if (get_ip_by_host(urlmap[i].true_url->host, &tmp_addr) == STS_FAILURE) {
         ...
         DEBUGC(DBCLASS_SIP, "sip_find_direction: reghost:%s ip:%s", urlmap[i].true_url->host, utils_inet_ntoa(from->sin_addr));

         if (memcmp(&tmp_addr, &from->sin_addr, sizeof(tmp_addr)) == 0) {
            if (MSG_IS_REQUEST(ticket->sipmsg)) {
               type=REQTYP_OUTGOING;
//     			DEBUGC(DBCLASS_SIP, "%s %d DDNDEBUG: type REQTYP_OUTGOING", __FILE__, __LINE__);
            } else {
...
//    			DEBUGC(DBCLASS_SIP, "%s %d DDNDEBUG: type REQTYP_OUTGOING", __FILE__, __LINE__);
   ...
   if (type == DIRTYP_UNKNOWN) {
     ...
     //DEBUGC(DBCLASS_SIP, "%s %d DDNDEBUG: type is still DIRTYP_UNKNOWN", __FILE__, __LINE__);

//     	DEBUGC(DBCLASS_SIP, "DDNDEBUG: Counting from %d to %d ", beg_idx, end_idx-1);
...
//     			DEBUGC(DBCLASS_SIP, "%s %d DDNDEBUG: type REQTYP_INCOMING", __FILE__, __LINE__);
                type=REQTYP_INCOMING;
				
				/*DDN: Save the invite call id*/
				if(is_it_invite)
				{
   					osip_to_t *tmpto;
   					osip_uri_param_t *tmptotag; /* gname, gvalue */

   					tmpto=request->to;
   					osip_to_get_tag(tmpto, &tmptotag);

					if(tmptotag &&  tmptotag->gvalue)
					{
						call_id_found = 0;
						for(bye_idx = 0; bye_idx < CALL_ID_MAP_SIZE; bye_idx++)
						{
							int tmp_idx = call_id_map[bye_idx].idx;
							if(urlmap[tmp_idx].active && strcmp(call_id_map[bye_idx].call_id, request->call_id->number) == 0)
							{
								//Found right destination
								i = call_id_map[bye_idx].idx;
								call_id_found = 1;

//								DEBUGC(DBCLASS_SIP, "DDNDEBUG: type REQTYP_INCOMING %s. "
//									"Call id(%s) at idx(%d/%d)", "RE-INVITE",
//									call_id_map[bye_idx].call_id, bye_idx, i);

								break;
							}
						}

						if(call_id_found)break;
					}
					else
					{
						sprintf(call_id_map[call_id_map_idx].call_id, "%s", request->call_id->number);
						call_id_map[call_id_map_idx].idx = i;
	
//						DEBUGC(DBCLASS_SIP, "DDNDEBUG: type REQTYP_INCOMING. INVITE. "
//							"Added callid(%s) at idx(%d)", request->call_id->number, call_id_map_idx);
	
						call_id_map_idx++;
	
						if(call_id_map_idx >= CALL_ID_MAP_SIZE){
							call_id_map_idx = 0;
						}
					}
				...
				else if(is_it_bye || is_it_ack || is_it_notify)
				{
					call_id_found = 0;
					for(bye_idx = 0; bye_idx < CALL_ID_MAP_SIZE; bye_idx++)
					{
						...
						if(urlmap[tmp_idx].active && strcmp(call_id_map[bye_idx].call_id, request->call_id->number) == 0)
						{
							...
							DEBUGC(DBCLASS_SIP, "DDNDEBUG: type REQTYP_INCOMING %s. "
								"Call id(%s) at idx(%d/%d)", (is_it_bye)?"BYE":"ACK/NOTIFY",
								call_id_map[bye_idx].call_id, bye_idx, i);
            ...
            if ((compare_url(response->from->url, urlmap[i].reg_url)==STS_SUCCESS) ||
				...
				call_id_found = 0;
     			...
     			DEBUGC(DBCLASS_SIP, "DDNDEBUG: type RESTYP_INCOMING");
                type=RESTYP_INCOMING;

				for(bye_idx = 0; bye_idx < CALL_ID_MAP_SIZE; bye_idx++)
				{
					if(strcmp(call_id_map[bye_idx].call_id,  request->call_id->number) == 0)
					{
						//Found right destination
						i = call_id_map[bye_idx].idx;
						DEBUGC(DBCLASS_SIP, "DDNDEBUG: type REQTYP_INCOMING. BYE. Found at idx(%d)", i);
						call_id_found = 1;
						break;
					}
				}
   ...
   if ((type == DIRTYP_UNKNOWN) && (MSG_IS_REQUEST(ticket->sipmsg))) {
     ...
     //DEBUGC(DBCLASS_SIP, "DDNDEBUG: type is still DIRTYP_UNKNOWN");
      ...
      for (i=beg_idx; i<end_idx; i++) {
         ...
         if ((compare_url(request->req_uri, urlmap[i].masq_url)==STS_SUCCESS) ||
            ...
            type=REQTYP_INCOMING;
     			DEBUGC(DBCLASS_SIP, "DDNDEBUG: type REQTYP_INCOMING");
			...
			if(is_it_invite)
			{
				...
				call_id_map[call_id_map_idx].idx = i;
     			...
     			DEBUGC(DBCLASS_SIP, "DDNDEBUG: type REQTYP_INCOMING. INVITE. "
							"Added callid(%s) at idx(%d)", request->call_id->number, i);
   ...
   if (type == DIRTYP_UNKNOWN) {
	 ...
	 if (MSG_IS_REQUEST(ticket->sipmsg)) {
            type=REQTYP_OUTGOING;
     			...
     			DEBUGC(DBCLASS_SIP, "%s %d DDNDEBUG: type RESTYP_OUTGOING", __FILE__, __LINE__);
            type=RESTYP_OUTGOING;
	 }
      }
   } /* if type == DIRTYP_UNKNOWN */
