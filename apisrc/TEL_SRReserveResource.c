/*-----------------------------------------------------------------------------
Program:        TEL_SRReserveResource.c 
Purpose:        Reserve both server side and client side licenses.
Author:         Anand Joglekar
Date:			10/01/2002
-----------------------------------------------------------------------------*/
#include "TEL_Common.h"

int TEL_SRReserveResource(int *zpVendorErrorCode)
{
	static char mod[]="TEL_SRReserveResource";
	char apiAndParms[MAX_LENGTH] = "";
	int rc;

	struct MsgToApp response;
	struct Msg_SRReserveResource lMsg;
	struct MsgToDM msg; 
	...
	*zpVendorErrorCode = 0;
	...
	rc = apiInit(mod, TEL_SRRESERVERESOURCE, apiAndParms, 1, 0, 0); 
	if (rc != 0) HANDLE_RETURN(rc);
 
	lMsg.opcode = DMOP_SRRESERVERESOURCE;
	...
	memcpy(&msg, &lMsg, sizeof(struct Msg_SRReserveResource));
