...
Update: 10/24/2012 djb MR 4096.  Fixed extra 0 for thousands.
------------------------------------------------------------------------------*/
...
//printf("RGDEBUG::%d::%s::found streaming request\n", __LINE__, __FUNCTION__);flush(stdout);
         ...
         ( synchronous == CLEAR_QUEUE_ALL ) )
    {
            ...
            rc = sendRequestToTTSClient((const ARC_TTS_REQUEST_SINGLE_DM *)&ttsRequest);
    }
//printf("RGDEBUG::%d::%s::found streaming request\n", __LINE__, __FUNCTION__);fflush(stdout);
...
--------------------------------------------------------------------------*/
static int make_date_list(mod, informat,outformat,string,list)
...
//		(me and Harshal) were not able to reproduce the arcML issue, so
	...
	if (number >=  a_billion)
	{
		...
		if ( number-(((long)number/a_billion)*a_billion) == 0 )
		{
			return(0);
		}
	...
	else if (number >= a_million)
	{
		...
		if (number-(((long)number/a_million)*a_million) == 0)
		{
			return(0);
		}
	...
	else if (number >= a_thousand)
	{
		...
		if ( number-(((long)number/a_thousand)*a_thousand) == 0 )
		{
			return(0);
		}
                                ...
                                int zMode, char *zDataOverrides)
{
        ...
        pTmpTag = ( PhraseTagList *)&GV_AppTags;
