...
Update: 06/22/10 djb Added TEL_BlastDial logic.
Update: 06/22/10 djb Added TEL_BlastDial logic.
Update: 09/10/13 djb Added network transfer logic
Update: 09/24/2014 djb MR 3839 - removed references to MRCPv1
Update: 08/19/2015 djb Added prepLongDestinationForFileIPC()
------------------------------------------------------------------------------*/
#define GLOBAL
#define TEL_COMMON
...
static int startCDRWritten = 0;
static int endCDRWritten = 0;
...
This should eventually call LogARCMsg.
----------------------------------------------------------------------------*/
int telCDRLog (char *mod, int mode, int msgId, int msgType, char *msgFormat, ...)
{
  va_list ap;
  char m[1024];
  char m1[512];
...
/*
 * MR 4802
 * GV_TerminatedViaExitTelecom = 0, 01 -> app disconnected
 * GV_TerminatedViaExitTelecom = 1, 02-> remote disconnected
 * GV_TerminatedViaExitTelecom = 3, 03 -> call was blind transfer
 */
  if (GV_TerminatedViaExitTelecom == 1)
  {
    sprintf (finalStatus, "%s", "01");          //  app disconnected
  }
  else if ( GV_TerminatedViaExitTelecom == 3 )
  {
    sprintf (finalStatus, "%s", "03");          //  call was blind transfer
  }
  else
  {
    sprintf (finalStatus, "%s", "02");          // remote disconnected
  }
...
readResponseFromDynMgr (char *mod, int timeout, void *pMsg, int zMsgLength)
{
  ...
  else
  {
    telVarLog (mod, REPORT_DETAIL, TEL_BASE, GV_Info,
             "[%s, %d] Received %d bytes. "
             "msg={op:%d,c#:%d,pid:%d,ref:%d,pw:%d,rc:%d,data:X}",
			 __FILE__, __LINE__, 
             rc,
             response->opcode,
             response->appCallNum,
             response->appPid,
             response->appRef,
             response->appPassword, response->returnCode);
  }
  return (0);
}

...
// #endif // VOICE_BIOMETRICS
    ...
    if (res->opcode == DMOP_DISCONNECT) {
      ...
      if (res->appCallNum == GV_AppCallNum1) {
        if (GV_BLegCDR == 1 && GV_Connected2 == 1) {
          ...
          setTime (GV_BridgeDisconnectTime, &GV_BridgeDisconnectTimeSec);
        }

        if (res->alternateRetCode == DMOP_TRANSFERCALL)			// MR 4802
        { 
              telVarLog (mod, REPORT_VERBOSE, TEL_CANT_OPEN_FIFO, GV_Info,
                "[%d] alternateRetCode=%d  DMOP_TRANSFERCALL=%d", __LINE__, res->alternateRetCode, DMOP_TRANSFERCALL);
        
            GV_TerminatedViaExitTelecom = 3;        
        }    
        ...
        if (GV_BLegCDR == 1 && GV_Connected1 != 0) {
          char portString[32];
          char lMsg[512];
			long bridgeEndTimeSec = 0L;
			char bridgeEndTime[32];
  ...
  default:
    ...
    default:
      ...
      if ( ((req->opcode == DMOP_SENDFAX) || (req->opcode == DMOP_RECVFAX) ) &&
           (res->opcode == DMOP_FAX_PLAYTONE) )
	  {
        break;
      }
	  else
      {
    	if (res->appPid != req->appPid) {
      		sprintf (Msg, TEL_UNEXPECTED_RESPONSE_MSG,
               res->opcode, res->appPid, res->appRef,
               req->opcode, req->appPid, req->appRef);
	      	telVarLog (mod, REPORT_NORMAL, TEL_BASE, GV_Err, Msg);		// BT-40
		}
      	return (-1);
	  }
...
sendRequestToDynMgr (char *mod, struct MsgToDM *request)
{
  ...
  sprintf (Msg, "[%s, %d] Sent %d bytes to (%s). "
           "msg={op:%d,c#:%d,pid:%d,ref:%d,pw:%d",
			__FILE__, __LINE__, rc, GV_RequestFifo,
           request->opcode,
           request->appCallNum,
...
int prepLongDestinationForFileIPC(char *zLabel, char *zFileContents, char *zNewDestination)
{
	...
	FILE		*fp;
	static char	mod[]="prepLongDestinationForFileIPC";

	sprintf(yStrFileName, "/tmp/%s.%d.%d", zLabel, GV_AppCallNum1, getpid());
	if((fp = fopen(yStrFileName, "w+")) == NULL)
	{
		telVarLog(mod,REPORT_NORMAL,TEL_BASE,GV_Err, "Failed to open file (%s) for output. [%d, %s]",
				yStrFileName, errno, strerror(errno));
		return(-1);
	}

	fprintf(fp, "%s", zFileContents);
	
	fclose(fp);
	sprintf(zNewDestination, "FILE:%s", yStrFileName);
	telVarLog(mod,REPORT_VERBOSE, TEL_BASE, GV_Info,
				"Wrote ipAddress field to file (%s) for IPC.", yStrFileName);

	return(0);
} // END: prepLongDestinationForFileIPC()
         ...
         int initRequired, int party1Required, int party2Required)
{
  ...
  {
	if ( apiId == TEL_EXITTELECOM )		// MR 5043
	{
		telVarLog(mod,REPORT_NORMAL,TEL_TELECOM_NOT_INIT, GV_Warn,
...
saveTypeAheadData (char *mod, int callNum, char *data)
{
  ...
  {
    telVarLog (mod, REPORT_VERBOSE, TEL_BASE, GV_Info,
			"In saveTypeAhead; received dtmf data; ($HIDE_DTMF) enabled.");
  }
  if (callNum == GV_AppCallNum1) {
...
Date:		06/17/99
------------------------------------------------------------------------------*/
int get_name_value (char *section, char *name,
...
setCDRKey (char *mod, char *port_no)
{
  ...
  if ((ret = gethostname (host_name, sizeof (host_name) - 1)) == -1) {
    sprintf (Msg, "[%s, %d] gethostname failed. errno=%d.", __FILE__, __LINE__, errno);
...
//                    GV_Err, "[%d] DJB: wrote slot %d, type %d, value %d to resp shm.", __LINE__,
//					slot, type, value);
      ...
      if (holder[0] == termChar) {
        ...
        *termCharPressed = 1;
        ...
        if (strchr (allowableChars, holder[0]) != NULL) {
          ...
          pos++;
          telVarLog (mod, REPORT_NORMAL, TEL_BASE,
                     GV_Info, "Invalid character <%c>.", holder[0]);
		  return(TEL_INVALID_INPUT);
		  break;
        }
      }
    }
...
get_first_phrase_extension (char *mod, char *directory, char *extension)
{
...
#ifdef CODE_THAT_THROWS_BROKEN_PIPE_ERROR
  ...
  modifiedPclose (fp, lChildProcessPid);
	...
	{
    	telVarLog (mod, REPORT_NORMAL, TEL_BASE, GV_Err,
               "Directory (%s) appears to be inaccessible. errno=%d",  directory, errno);
    	return (-1);
	}
	else
	{
		...
		while (1)
		{
			...
			if( (pLastDot = strrchr(dirEntryPtr->d_name, '.')) != NULL)
			{
				if(strlen(pLastDot) <= 1)   //Proceed only if the file has extension
				{
					continue;
				}

				if ( strstr(pLastDot, ".tar") != NULL )
				{
					continue;
				}
...
sendRequestToSRClient (char *mod, struct MsgToDM *request)
{
  ...
  request->appPassword = request->appCallNum;
	...
	if ( (GV_SRFifoFd == -1) || GV_SRFifo[0] == '\0' )
	{
  ...
  if (rc == -1) {
    telVarLog (mod, REPORT_NORMAL, TEL_BASE, GV_Err,
               "Can't write message to (%s). [%d, %s]", GV_SRFifo, errno,
				strerror(errno));
    return (-1);
  }
  telVarLog (mod, REPORT_VERBOSE, TEL_BASE, GV_Info,
...
int openChannelToAIClient (char *mod, char *zAIClientFifo)
{
  ...
  char tmpMsg2[256];
	...
	if (zAIClientFifo[0] != '\0')
	{
        sprintf (GV_AIFifo, "%s", zAIClientFifo);
	}
	else
	{
		if (GV_AIFifo[0] == '\0')
		{
			memset ((char *) fifoDir, '\0', sizeof (fifoDir));
			if ((rc = UTL_GetArcFifoDir (AI_FIFO_INDEX, fifoDir, sizeof (fifoDir), Msg, sizeof (Msg))) != 0)
			{
				sprintf (fifoDir, "%s", "/tmp");
				sprintf (tmpMsg2, "Defaulting fifo directory to %s.  %s", fifoDir, Msg);
				telVarLog (mod, REPORT_VERBOSE, TEL_CANT_OPEN_FIFO, GV_Info, tmpMsg2);
			...
			sprintf (GV_AIFifo, "%s/%s", fifoDir, AI_REQUEST_FIFO);
		}
	}
	telVarLog(mod,REPORT_VERBOSE, TEL_BASE, GV_Info, "Set AI FIFO to (%s)", GV_AIFifo);
	
  if ((mkfifo (GV_AIFifo, S_IFIFO | 0666) < 0)
      && (errno != EEXIST)) {
    telVarLog (mod, REPORT_NORMAL,
               TEL_CANT_CREATE_FIFO, GV_Err,
               TEL_CANT_CREATE_FIFO_MSG, GV_AIFifo, errno, strerror (errno));
    return (-1);
  }

  if ((GV_AIFifoFd = open (GV_AIFifo, O_RDWR)) < 0) {
    telVarLog (mod, REPORT_NORMAL, TEL_CANT_OPEN_FIFO,
               GV_Err, TEL_CANT_OPEN_FIFO_MSG,
               GV_AIFifo, errno, strerror (errno));
    return (-1);
  }
  telVarLog (mod, REPORT_VERBOSE, TEL_BASE, GV_Info,
             "Successfully opened AIClient fifo (%s).  fd=%d",
             GV_AIFifo, GV_AIFifoFd);
  return (0);
}
...
int
sendRequestToAIClient (char *mod, struct MsgToDM *request)
{
#define MAX_GPT_FIFO_SIZE	255

	int rc;
	struct Msg_AIInit		aiInitMsg[260];
	struct Msg_AIProcessContent	aiProcessContent[260];
	struct Msg_AIExit		aiExitMsg[260];

  request->appPassword = request->appCallNum;
  char tmpMsg[256];
	char			msgToSend[MAX_GPT_FIFO_SIZE	+ 1] = 
		"                                                  "   // 50
		"                                                  "   // 100
		"                                                  "   // 100
		"                                                  "	// 200
	...
	if ( (GV_AIFifoFd == -1) || GV_AIFifo[0] == '\0' )
	{
...
//	memset(msgToSend, ' ', sizeof(msgToSend));
	switch(request->opcode)
	{
		case DMOP_AIINIT:
			memcpy(aiInitMsg, request, sizeof(struct MsgToDM));
			sprintf(tmpMsg, 
				"{\"opcode\":%d,\"appCallNum\":%d,\"appPid\":%d,\"appRef\":%d,\"appPassword\":%d,\"appFifo\":\"%s\"}",
					aiInitMsg->opcode, 
					aiInitMsg->appCallNum, 
					aiInitMsg->appPid, 
					aiInitMsg->appRef,
					aiInitMsg->appPassword,
					aiInitMsg->appFifo);
			break;
		case DMOP_AIPROCESS_CONTENT:
			memcpy(aiProcessContent, request, sizeof(struct MsgToDM));
			sprintf(tmpMsg, 
				"{\"opcode\":%d,\"appCallNum\":%d,\"appPid\":%d,\"appRef\":%d,\"appPassword\":%d,\"aiOpcode\":%d,\"content\":\"%s\"}",
					aiProcessContent->opcode, 
					aiProcessContent->appCallNum, 
					aiProcessContent->appPid, 
					aiProcessContent->appRef,
					aiProcessContent->appPassword,
					aiProcessContent->aiOpcode,
					aiProcessContent->content);
			break;
		case DMOP_AIEXIT:
			memcpy(aiExitMsg, request, sizeof(struct MsgToDM));
			sprintf(tmpMsg, 
				"{\"opcode\":%d,\"appCallNum\":%d,\"appPid\":%d,\"appRef\":%d,\"appPassword\":%d}",
					aiExitMsg->opcode, 
					aiExitMsg->appCallNum, 
					aiExitMsg->appPid, 
					aiExitMsg->appRef,
					aiExitMsg->appPassword);
			break;
	}


	memcpy(msgToSend, tmpMsg, strlen(tmpMsg));

  rc = write (GV_AIFifoFd, (char *) msgToSend, MAX_GPT_FIFO_SIZE);
  if (rc == -1) {
    telVarLog (mod, REPORT_NORMAL, TEL_BASE, GV_Err,
               "Can't write message to (%s). [%d, %s]", GV_AIFifo, errno,
				strerror(errno));
    return (-1);
  }
  telVarLog (mod, REPORT_VERBOSE, TEL_BASE, GV_Info,
