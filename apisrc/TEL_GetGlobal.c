...
Update: 071001 APJ Put a break after BEEP case.
Update: 071201 APJ Added support for SECONDS_FROM_ACCEPTANCE.
Update: 071201 APJ Added support for SECONDS_FROM_CONNECT.
Update: 071801 APJ Added support for DELIMITER.
Update: 070102 APJ Added support for BANDWIDTH.
...
Update:     11/07/2005 ddn added AUDIO_SAMPLING_RATE
...
static	char	*vars[] = {
			...
			"$BRIDGE_PORT",
			"$RECORD_TERM_CHAR",
			"$PLAY_BACK_BEEP_INTERVAL",		// 35
			"$MAX_PAUSE_TIME",		
			"$FAX_HDR_FMT",
			"$LAST_PROMPT_DURATION",
			"$RELEASE_CAUSE_CODE",
			"$LAST_STREAM_DURATION",		// 40
			"$SR_DTMF_ONLY",			
			"$AUDIO_SAMPLING_RATE",
			"$FAX_DEBUG",
			"$MAX_CALL_DURATION",
			"$LAST_PLAY_POSITION",			// 45
			"$CALL_PROGRESS_DETAIL",		
			"$DTMF_AVAILABLE",
			"$BRIDGE_CONNECT_TIME",
...
------------------------------------------------------------------------------*/
int TEL_GetGlobal(const char *var_name, int *var_value)
{
	static char mod[]="TEL_GetGlobal";
	char apiAndParms[MAX_LENGTH] = "";
	char apiAndParmsFormat[]="%s(%s,&value)";
	int	index = 0;
	int rc;
	time_t	now;
	...
	*var_value = 0;
	...
	switch(index)
	{
		...
		case 44: /* $MAX_CALL_DURATION */
			sprintf(msg.name, "%s", "$MAX_CALL_DURATION");
			break;

		case 45: /* $LAST_PLAY_POSITION */
			sprintf(msg.name, "%s", "$LAST_PLAY_POSITION");
			break;

		case 46: /* $CALL_PROGRESS_DETAIL */
			sprintf(msg.name, "%s", "$CALL_PROGRESS_DETAIL");
			break;

		case 47: /* $LAST_PLAY_POSITION */
			sprintf(msg.name, "%s", "$DTMF_AVAILABLE");
			break;
        case 48: /* $BRIDGE_CONNECT_TIME */
			*var_value = GV_BridgeConnectTimeSec;
			HANDLE_RETURN(0);
            break;
        case 49: /* BRIDGE_DISCONNECT_TIME */
			*var_value = GV_BridgeDisconnectTimeSec;
			HANDLE_RETURN(0);
            break;
    	case 50: 	/* $SR_TYPE */
			*var_value = GV_SRType;
    	...
    	default:
			telVarLog(ModuleName,REPORT_NORMAL,TEL_INVALID_PARM, GV_Err,
				"Variable <%s> is not a defined global variable name returning value = -1",
				var_name);
			*var_value = -1;
