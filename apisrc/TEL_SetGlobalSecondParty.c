/*-----------------------------------------------------------------------------
Program:	TEL_SetGlobalSecondParty.c
...
------------------------------------------------------------------------------*/
int TEL_SetGlobalSecondParty(const char *var_name, int var_value)
{
static char mod[]="TEL_SetGlobalSecondParty";
int	index = 0;
int	rc;
...
switch(index)
{
        ...
        case 44:    /* "$OVERRIDE_INBAND_DTMF" */			// BT-313
            sprintf(msg.name, "%s", "$OVERRIDE_INBAND_DTMF");
			if((var_value != YES) && (var_value != NO))
			{
				telVarLog(mod,REPORT_NORMAL,TEL_INVALID_PARM,GV_Err,
					"Invalid value for $OVERRIDE_INBAND_DTMF : %d. Valid values are "
					"YES, NO. Setting to NO (%d)", var_value, NO);
				HANDLE_RETURN(-1);
			}

			GV_OverrideInbandDTMF = var_value;
			sprintf(charValue, "%d", var_value);
