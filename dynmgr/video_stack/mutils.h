...
//   Byte   8 bit
//   BOOL   sizeof(BOOL) i.e. 1 byte or 4 bytes
//   char   1
//   short  2
//   int    4
...
/**
  ...
  * @hideinitializer
  * @param dest Destination as BOOL *
  * @param src  Source value as BOOL
  */
#define MBool2Bool(/* BOOL* */dest, /* BOOL */src)      if (src) {(dest) = TRUE;} else {(dest) = FALSE;}
...
/**
  ...
  */
#define M1Buf2Bool(/* BOOL */dest, /* MOCTET* */src)    (dest) = (BOOL)*(src); (src) += sizeof(MOCTET);
...
/**
  ...
  * @param src  Source value as BOOL
  */
#define MBool21Buf(/* MOCTET* */dest, /* BOOL */src)    if (src) {*(dest) = TRUE; } else {*(dest) = FALSE; } (dest) += sizeof(MOCTET);
...
extern M_IMPORT char*   MGetFileNameTitle(
    ...
    BOOL p_zForSave,                    ///< Flag indicating the file is for saving (TRUE) or opening (FALSE) 
...
extern M_IMPORT BOOL    MCenterWindow(HWND hwndChild, HWND hwndParent);
#endif
