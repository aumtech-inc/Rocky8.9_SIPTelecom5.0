...
#define rtpSleep(millisec, zLastSleepClock, zLine, zCall) rtpSleep_clock(millisec, zLastSleepClock, zLine, zCall)
...
//	ftime (&tb);
	...
	sprintf(tmpBuf1, "%d",  tb.tv_nsec);
	sprintf(tmpBuf2, "%.*s", 3, tmpBuf1);
	tb.tv_nsec = atoi(tmpBuf2);
...
static int rtpSleep_clock(int millisec, clock_t *zLastClock, int line, int zCall)
{
	...
	cpu_time_used_ms = ((double) (currentClock - *zLastClock)) / (CLOCKS_PER_SEC / 1000.0);	

	util_u_sleep ( (millisec * 1000) - cpu_time_used_ms);
...
int
rtpSleep_withbreaks (int sleepTime, struct timeval *zLastSleepTime,
					 int zCall, int interrupt_option, int zLine)
{
	int             rc = -1;
	char            mod[] = "rtpSleep_withbreaks";
	long            yIntSleepTime = 0;
	struct timeval	yCurrentTime;
...
//		zLastSleepTime->tv_nsec = zLastSleepTime->tv_nsec / 1000;

		current = (1000 * yCurrentTime.tv_sec) + yCurrentTime.tv_usec;
		last = (1000 * zLastSleepTime->tv_sec) + zLastSleepTime->tv_usec;
	...
	gettimeofday(zLastSleepTime, NULL);
	zLastSleepTime->tv_usec = zLastSleepTime->tv_usec / 1000;
...
int chopUtterance (int zCall, char *zFile, long startFrom)
{
	...
	{
		dynVarLog (__LINE__, zCall, mod, REPORT_DETAIL, TEL_BASE, INFO,
			"stat(%s) failed. [%d, %s]", zFile, errno, strerror(errno));
		return (-1);
	}
	dynVarLog (__LINE__, zCall, mod, REPORT_VERBOSE, TEL_BASE, INFO,
			   "Chopping %s; size is currently %ld. Will trim off front %d bytes.", zFile, yStat.st_size, startFrom);
	...
	{
		dynVarLog (__LINE__, zCall, mod, REPORT_DETAIL, TEL_BASE, INFO,
			"fopen(%s) failed. [%d, %s]", yStrBackupFile, errno, strerror(errno));
		return (-1);
	}
#endif
	...
	if (fp_read == NULL)
	{
		fclose (fp_write);
		dynVarLog (__LINE__, zCall, mod, REPORT_DETAIL, TEL_BASE, INFO,
			"fopen(%s) failed. [%d, %s]", zFile, errno, strerror(errno));
		return (-1);
	}
	...
	{
		dynVarLog (__LINE__, zCall, mod, REPORT_DETAIL, TEL_BASE, INFO,
			"failed to malloc. [%d, %s]", errno, strerror(errno));
		fclose (fp_write);
		fclose (fp_read);
		return (-1);
	}
	...
	{
		dynVarLog (__LINE__, zCall, mod, REPORT_DETAIL, TEL_BASE, INFO,
			"fread(%s) failed. [%d, %s]", zFile, errno, strerror(errno));
		fclose (fp_write);
		fclose (fp_read);
		free (buf);
	...
	if ((yIntFwrite =
		 fwrite (buf, 1, yContentToWrite, fp_write)) < 0)
	{
		dynVarLog (__LINE__, zCall, mod, REPORT_DETAIL, TEL_BASE, INFO,
			"fwrite(%s) failed. [%d, %s]", yStrBackupFile, errno, strerror(errno));
		fclose (fp_write);
	...
	{
		dynVarLog (__LINE__, zCall, mod, REPORT_DETAIL, TEL_BASE, INFO,
			"After trimming, stat(%s) failed. [%d, %s]", zFile, errno, strerror(errno));
		return (0);
	}
	dynVarLog (__LINE__, zCall, mod, REPORT_VERBOSE, TEL_BASE, INFO,
			   "Chopped %s; size is now %ld.", zFile, yStat.st_size);
	return (0);
...
int makeDuplicateWavFile (int zCall, char *zFile)
{
	...
	{
		dynVarLog (__LINE__, zCall, mod, REPORT_DETAIL, TEL_BASE, INFO,
			"stat(%s) failed. [%d, %s] Unable to make backup of utterance file.", zFile, errno, strerror(errno));
		return (-1);
	}
	...
	{
		dynVarLog (__LINE__, zCall, mod, REPORT_DETAIL, TEL_BASE, INFO,
			"fopen(%s) failed. [%d, %s] Unable to make backup of utterance file.", yStrBackupFile, errno, strerror(errno));
		return (-1);
	}
	...
	if (fp_read == NULL)
	{
		fclose (fp_write);
		dynVarLog (__LINE__, zCall, mod, REPORT_DETAIL, TEL_BASE, INFO,
			"fopen(%s) failed. [%d, %s] Unable to make backup of utterance file.", zFile, errno, strerror(errno));
		return (-1);
	}
	...
	{
		dynVarLog (__LINE__, zCall, mod, REPORT_DETAIL, TEL_BASE, INFO,
			"failed to malloc. [%d, %s] Unable to make backup of utterance file (%s).", errno, strerror(errno), zFile);
		fclose (fp_write);
		fclose (fp_read);
		return (-1);
	}
	...
	{
		dynVarLog (__LINE__, zCall, mod, REPORT_DETAIL, TEL_BASE, INFO,
			"fread(%s) failed. [%d, %s] Unable to make backup of utterance file.", zFile, errno, strerror(errno));
		fclose (fp_write);
		fclose (fp_read);
		free (buf);
		arc_free (zCall, mod, __LINE__, buf, yStat.st_size + 1);
	...
	{
		dynVarLog (__LINE__, zCall, mod, REPORT_DETAIL, TEL_BASE, INFO,
			"fwrite(%s) failed. [%d, %s] Unable to make backup of utterance file.", yStrBackupFile, errno, strerror(errno));
		fclose (fp_write);
		fclose (fp_read);
	...
	dynVarLog (__LINE__, zCall, mod, REPORT_DETAIL, TEL_BASE, INFO,
			   "For debug, copied file utterance file (%s) to (%s)", zFile, yStrBackupFile);
	return (0);
...
trimStartOfWavFile (int zCall, char *zFile, time_t t1, time_t t2)
{
	...
	int             ySecondsToBeTrimmed = (t2 - t1);
//	int             yBytesToBeTrimmed = (ySecondsToBeTrimmed * 8192) - 8192;
	int             yBytesToBeTrimmed = ySecondsToBeTrimmed * 8192;

	dynVarLog (__LINE__, zCall, mod, REPORT_VERBOSE, TEL_BASE, INFO,
			   "ySecondsToBeTrimmed=%d", ySecondsToBeTrimmed);
#if 0
	// BT-272
	if ( yBytesToBeTrimmed > 16384 )
	{
		yBytesToBeTrimmed -= 8192;
		ySecondsToBeTrimmed -= 1;
	}
#endif
...
//	int             yBytesToBeTrimmed = (ySecondsToBeTrimmed * 8192) - 8192;
	...
	int             ySecondsToBeTrimmed = (t2 - t1);
	int             yBytesToBeTrimmed = (ySecondsToBeTrimmed * 8192) + 4096;
...
//fprintf(stderr, " %s() gCall[zCall].msgRecord.terminate_char==%d", __func__, gCall[zCall].msgRecord.terminate_char);
	...
	char            dtmfToSend = gCall[zCall].dtmfInCache[i];
		...
		if (gCall[zCall].isBeepActive == 1)
		{
			...
			rtpSleep (20, &gCall[zCall].lastInRtpClock, __LINE__, zCall);
			continue;
		}
...
//DDN 06/24/2011    Inband DTMF Detection
	...
	struct timespec    currentDtmfTime;
	struct timespec yCurrentTime;
	long            current = 0;
	long            last = 0;
	char			tmpBuf1[32];
	char			tmpBuf2[32];

			clock_gettime(CLOCK_REALTIME, &currentDtmfTime);
			sprintf(tmpBuf1, "%ld",  currentDtmfTime.tv_nsec);
			sprintf(tmpBuf2, "%.*s", 3, tmpBuf1);
			currentDtmfTime.tv_nsec = atoi(tmpBuf2);

			current = (1000 * currentDtmfTime.tv_sec) + currentDtmfTime.tv_nsec;
			last =
				(1000 * arcDtmfData[zCall].lastTime.tv_sec) +
				arcDtmfData[zCall].lastTime.tv_nsec;
...
//				ftime (&arcDtmfData[zCall].lastTime);
				clock_gettime(CLOCK_REALTIME, &arcDtmfData[zCall].lastTime);
...
//				   "[%d] DJB: zMsgToDM is NULL.", zLine);
		(void) sendByeToGoogleSRFifo(zCall, __LINE__);
...
//              clearSpeakList(zCall, __LINE__);
	...
	__DDNDEBUG__ (DEBUG_MODULE_SR, "MRCP: ARCGS: Sending silence to Google SR Client", "", zCall);
								...
								rtpSleep (20, &gCall[zCall].lastOutRtpClock, __LINE__, zCall);
...
//__DDNDEBUG__(DEBUG_MODULE_SR, yStrTmpLogMsg, "", zCall);
	...
	int             yTmpCounter = 50;
		...
		if (gCall[zCall].gUtteranceFileFp != NULL)
		{
			...
			if ( gCall[zCall].utterance_location == 0 )
			{
			...
			{
				if (  gCall[zCall].utterance_location > (8192 + 4096) ) 			// BT-272
				{
					if ( access ("/tmp/utteranceDebug.txt", F_OK) == 0 )
					{
						makeDuplicateWavFile (zCall, gCall[zCall].lastRecUtteranceFile);
					}
					chopUtterance (zCall, gCall[zCall].lastRecUtteranceFile, gCall[zCall].utterance_location - 8192); // 1 second prior
				}
				else
				{
					dynVarLog (__LINE__, zCall, mod, REPORT_VERBOSE, TEL_BASE, INFO,
					   "[%d] Not trimming utterance file (%s) since utterance started < 1 second from start (%d).", zLine,
						gCall[zCall].lastRecUtteranceFile, gCall[zCall].utterance_location);
				}
			}
		}
...
//		gCall[zCall].speechRec = 1;
		...
		sprintf(gsDir, "%s", "/tmp/.googleAudio");
		...
		dynVarLog (__LINE__, zCall, mod, REPORT_VERBOSE, TEL_BASE, INFO,
				"yMsgRecognize.lead_silence:%d  gCall[%d].msgRecord.lead_silence:%d totalTime=%d",
	...
	dynVarLog (__LINE__, zCall, mod, REPORT_VERBOSE, TEL_BASE, INFO,
			   "[%d] RECORD START with audioRecordFilename=(%s); beepFile=(%s) ", __LINE__,
...
//					zCall, gCall[zCall].msgToDM.opcode);
	...
	clock_gettime(CLOCK_REALTIME, &tb);
	sprintf(tmpBuf1, "%ld",  tb.tv_nsec);
	sprintf(tmpBuf2, "%.*s", 3, tmpBuf1);
	tb.tv_nsec = atoi(tmpBuf2);

	currentMilliSec = (tb.tv_sec + ((double) tb.tv_nsec) / 1000) * 1000;
	...
	if ( zMsgToDM->opcode == DMOP_SRRECOGNIZE )
	{
		gCall[zCall].googleRecordEndTime = yCurrentTime + 20;  // temporary; will correct it at ACTIVE_SPEAKER
	}
...
//			gCall[zCall].msgRecord.lead_silence);
	     ...
	     ( gCall[zCall].msgRecord.lead_silence > 0) )
	{
		if (gCall[zCall].codecType == 0 || gCall[zCall].codecType == 8)
		{
			...
			{
				// djb - for googleRecord, the inband tone detection is started at the PLAY END
				if ( gCall[zCall].googleRecordIsActive == 0 )
				{
	...
	while (1)
	{
		...
		{
			time_t tmpRecordTime;

			if ( ( gCall[zCall].googleRecordIsActive == 1 ) ||
			     ( zMsgToDM->opcode == DMOP_SRRECOGNIZE ) )
			{
				tmpRecordTime = gCall[zCall].googleRecordEndTime;
			}
			else
			{
				tmpRecordTime = yRecordTime;
			}

			if (yCurrentTime >= tmpRecordTime)
			{
				response.opcode = gCall[zCall].msgRecord.opcode;
				yRc = ACTION_GET_NEXT_REQUEST;
				dynVarLog (__LINE__, zCall, mod, REPORT_VERBOSE, TEL_BASE, INFO,
						"Max record time reached.  Calling break.");
				break;
			}
		}
...
//					gCall[zCall].msgRecord.trail_silence);
	...
	else if ( zMsgToDM->opcode == DMOP_SRRECOGNIZE )
	{
		gCall[zCall].googleHasPromptPlayed = 0;
		(void)sendByeToGoogleSRFifo(zCall, __LINE__);
	}
	...
	if (zMsgToDM == NULL)
	{
		...
		while (1)
		{
			...
			if (gCall[zCall].isSendRecvFaxActive == 1)
			{
				...
				rtpSleep (200, &gCall[zCall].lastOutRtpClock, __LINE__, zCall);
				continue;
			}
...
//              zCall, gCall[zCall].speechRecFromClient);
				...
				gCall[zCall].rtp_session_mrcp == NULL)
			{
				...
				dynVarLog (__LINE__, zCall, mod, REPORT_VERBOSE, TEL_BASE,
						   INFO, "[%d] Setting dtmfAvailable to 0.", zLine);
// BT-263		gCall[zCall].dtmfAvailable = 0;
...
//										 __func__, speedBytes);
	...
	int             len = speedBytes - gCall[zCall].codecBufferSize;
			...
			if (audio_opts & SPEAKFILE_AUDIO_PROCESS_SPEED)
			{
				//speedSleep ? rtpSleep (sleep_time, &gCall[zCall].lastOutRtpTime, __LINE__, zCall) : 0;
				speedSleep ? rtpSleep (sleep_time, &gCall[zCall].lastOutRtpClock, __LINE__, zCall) : 0;
			}
			else
			{
				//rtpSleep (sleep_time, &gCall[zCall].lastOutRtpTime, __LINE__, zCall);
				rtpSleep (sleep_time, &gCall[zCall].lastOutRtpClock, __LINE__, zCall);
			}
...
inputThread (void *z)
{
	...
	dynVarLog (__LINE__, zCall, mod, REPORT_VERBOSE, TEL_BASE, INFO, "pid=%d", getpid());
		...
		else if (yInputAction.actionId == ACTION_CONTINUE)
		{
			...
			case DMOP_RECORDMEDIA: 
				if ( gCall[zCall].googleRecord == YES )
				{
					...
					gCall[zCall].googleRecordIsActive = 1;
					gCall[zCall].googleHasPromptPlayed = 0;
					time (&gCall[zCall].googleRecordEndTime);
					gCall[zCall].googleRecordEndTime += 20;  // temporary; will correct it at PLAY END
				}
...
//							gCall[zCall].toneDetectionBuff.leading_samples);
						...
						if ( ! gCall[zCall].googleRecordEndTimeIsSet )
						{
							gCall[zCall].googleRecordEndTimeIsSet = 1;
							time (&gCall[zCall].googleRecordEndTime);
							dynVarLog (__LINE__, zCall, mod, REPORT_VERBOSE, TEL_BASE, INFO,
													"Setting googleRecordEndTime from now + %d seconds.",
													gCall[zCall].msgRecord.record_time);
							gCall[zCall].googleRecordEndTime += gCall[zCall].msgRecord.record_time;
						}
					}

					if ( ! gCall[zCall].googleHasPromptPlayed )
					{
						gCall[zCall].googleHasPromptPlayed = 1;

						dynVarLog (__LINE__, zCall, mod, REPORT_VERBOSE, TEL_BASE, INFO,
							"googleStreamingSR:%d, googleBarginDidOccur:%d, googleRecordIsActive:%d",
							gCall[zCall].googleStreamingSR,
						   	gCall[zCall].googleBarginDidOccur,
							gCall[zCall].googleRecordIsActive);
						     ...
						     ( gCall[zCall].googleRecordIsActive == 1 ) )
						{
							...
							if ( ( gCall[zCall].googleRecordIsActive == 0 ) &&  // google steaming
							     ( gCall[zCall].googleAlreadyRepliedToApp == 0 ) )
							{
							    gCall[zCall].googleAlreadyRepliedToApp = 1;
							...
							else		// google record
							{
								time (&gCall[zCall].googleRecordEndTime);
								dynVarLog (__LINE__, zCall, mod, REPORT_VERBOSE, TEL_BASE, INFO,
										"Setting googleRecordEndTime %ld + %d",
										gCall[zCall].googleRecordEndTime,
										gCall[zCall].msgRecord.record_time);
		   ...
		   int canDeleteResponseFifo)
{
   ...
   clearPortLock(zCall);
	...
	/* Clock Sleep */
	gCall[zCall].lastOutRtpClock = 0;
	gCall[zCall].lastInRtpClock = 0;

	/*RTP Sleep */
	gCall[zCall].lastOutRtpTime.tv_usec = 0;
	gCall[zCall].lastInRtpTime.tv_usec = 0;

	gCall[zCall].lastOutRtpTime.tv_sec = 0;
	gCall[zCall].lastInRtpTime.tv_sec = 0;
	...
	gCall[zCall].gUtteranceFileFp = NULL;
	gCall[zCall].utterance_location = 0;
...
//	fprintf(stderr, " %s(%d) freeing decoder resources called from %s\n", __func__, zCall, mod);
	...
	memset (&gCall[zCall].googleOptions, 0, sizeof(gCall[zCall].googleOptions));
...
//						gCall[zCall].toneDetectionBuff.leading_samples);
	...
	int             i;
		...
		if (detect & ARC_TONES_BEGINNING_AUDIO_TIMEOUT)
		{
			...
			if(gCall[zCall].googleUDPFd > -1)
			{
				(void) sendByeNoWaitToGoogleSRFifo(zCall, __LINE__);
			}
			if ( ( gCall[zCall].googleStreamingSR == 1 ) ||
			     ( gCall[zCall].googleRecordIsActive == 1 ) )
			{
		...
		if (detect & ARC_TONES_ACTIVE_SPEAKER){
			...
			if((gCall[zCall].GV_LastCallProgressDetail & ARC_TONES_ACTIVE_SPEAKER) == 0)
			{
				...
				if ( gCall[zCall].googleStreamingSR == 1 )
				{
					...
					if ( gCall[zCall].googleAlreadyRepliedToApp == 0 )
					{
					    gCall[zCall].googleAlreadyRepliedToApp = 1;
						gCall[zCall].googleSRResponse.returnCode = 0;
...
googleStartInbandToneDetectionThread (int zCall, int zLine, int mode)
{
	...
	if (mode & ARC_TONES_BEGINNING_AUDIO_TIMEOUT)
	{
		// djb - Adding 30 extra seconds is to essentially disable
		//       leading timeout. This is for Google Streaming, and 
		//       the prompt has to be played.  
		//
		//       We're just listening for ACTIVE_SPEAKER
		//
		arc_tones_set_silence_params (&gCall[zCall].toneDetectionBuff,
									  gToneDetectionNoiseThresh,
		...
		dynVarLog (__LINE__, zCall, (char *) __func__, REPORT_VERBOSE, TEL_BASE, INFO,
				"Set lead silence to %d ( +30 ); trail silence to %d.",
				gCall[zCall].msgRecord.lead_silence,
				gCall[zCall].msgRecord.trail_silence);
	}
...
process_DMOP_SETGLOBALSTRING (int zCall)
{
	...
	else if (strcmp (yMsgSetGlobalString.name, "$GOOGLE_OPTIONS") == 0)
	{
		dynVarLog (__LINE__, zCall, mod, REPORT_VERBOSE, FAX_BASE, INFO,
				   "Setting googleOptions to [%s].",
				   yMsgSetGlobalString.value);
		snprintf (gCall[zCall].googleOptions,
				  sizeof (gCall[zCall].googleOptions), "%s", yMsgSetGlobalString.value);
	}
	else
	{
		dynVarLog (__LINE__, zCall, mod, REPORT_NORMAL, TEL_INVALID_PARM, ERR,
...
process_DMOP_SPEECHDETECTED (int zCall)
{
	...
	if (gCall[zCall].gUtteranceFileFp != NULL)
	{
		gCall[zCall].utterance_location = ftell(gCall[zCall].gUtteranceFileFp);
		dynVarLog (__LINE__, zCall, mod, REPORT_VERBOSE, TEL_BASE, INFO,
			"Set utterance location to %ld.", gCall[zCall].utterance_location);
	}
	time (&tb);
...
void *googleReaderThread(void *z)
{
	...
	while(gCanExit == 0)
	{
		...
		yRc = read (gGoogleSRResponseFifoFd, &gsr, sizeof (_GSResponse));
//		dynVarLog (__LINE__, gsr.telport, mod, REPORT_VERBOSE, TEL_BASE, INFO,
...
{
	static char mod[]="sendByeToGoogleSRFifo";
...
int sendByeNoWaitToGoogleSRFifo(int zCall, int zLine)
{
	...
	if(gCall[zCall].googleUDPFd > -1)
	{
		__DDNDEBUG__ (DEBUG_MODULE_SR, "MRCP: ARCGS: Closing googleUDPFd", "", gCall[zCall].googleUDPFd);
		dynVarLog (__LINE__, zCall, mod, REPORT_VERBOSE, TEL_BASE, INFO,
					"[%d] Sending 'bye-nowait' to client.", zLine);
...
} // END: sendByeNoWaitToGoogleSRFifo()
...
int sendRequestToGoogleSRFifo(int zCall, int zLine, int gsopcode, char *recordedFile)
{
	...
	memset((_GSRequest *)&gsr, '\0', sizeof(gsr));
	gsr.opcode 	= gsopcode;
	gsr.mmid 	= gDynMgrId;
	gsr.telport = zCall;
	gsr.rectime = 30;
	...
	if ( ( gsopcode == GSR_RECORD_REQUEST ) ||
	     ( gsopcode == GSR_STREAMING_REQUEST ) )
	{
		if ( gCall[zCall].googleOptions[0] != '\0' )
		{
			sprintf(gsr.options, "%s", gCall[zCall].googleOptions );
			memset (&gCall[zCall].googleOptions, 0, sizeof(gCall[zCall].googleOptions));
		}
	}
	...
	dynVarLog (__LINE__, zCall, mod, REPORT_VERBOSE, TEL_BASE, INFO,
		"[%d] Sending %d bytes to GoogleSRRequestFifo fd(%d) for %d,%d,%d,%d,%d,%s,%s,%s", zLine,
		yRc, gGoogleSRRequestFifoFd,
		gsr.opcode,
		gsr.mmid,
		gsr.telport,
		gsr.rectime,
	...
	yRc = write (gGoogleSRRequestFifoFd, &gsr, sizeof (_GSRequest));
	
	pthread_mutex_unlock (&gCall[zCall].gMutexSentGoogleRequest);

	dynVarLog (__LINE__, zCall, mod, REPORT_VERBOSE, TEL_BASE, INFO,
		"[%d] Sent %d bytes to GoogleSRRequestFifo fd(%d)", zLine,
		yRc, gGoogleSRRequestFifoFd);
...
#endif
		fprintf (stdout,
				 "ARC Media Manager for SIP Telecom Services on %s.\n"
				 "Compiled on %s %s.\n", yOSVersion, __DATE__, __TIME__);
...
//Initialize Aculab
	...
	for (i = gStartPort; i <= gEndPort; i++)
	{
		...
		/*RTP Sleep */
		gCall[i].lastOutRtpTime.tv_sec = 0;
		gCall[i].lastInRtpTime.tv_sec = 0;
		...
		gCall[i].googleAlreadyRepliedToApp = 0;
...
//              MAX_AVB_DATA + 1, gCall[i].avb_bufferPCM);
		...
		memset (&gCall[i].googleOptions, 0, sizeof(gCall[i].googleOptions));
					...
					struct dtmf_data_holder *zDtmfData, int processType)
{
	...
	char			tmpBuf2[64];
					...
					clock_gettime(CLOCK_REALTIME, &currentDtmfTime);
					sprintf(tmpBuf1, "%d",  currentDtmfTime.tv_nsec);
					sprintf(tmpBuf2, "%.*s", 3, tmpBuf1);
					currentDtmfTime.tv_nsec = atoi(tmpBuf2);

					current =
						(1000 * currentDtmfTime.tv_sec) +
						currentDtmfTime.tv_nsec;

					last =
						(1000 * zDtmfData->lastTime.tv_sec) +
						zDtmfData->lastTime.tv_nsec;
					...
					else
					{
						...
						clock_gettime(CLOCK_REALTIME, &zDtmfData->lastTime);
						sprintf(tmpBuf1, "%d",  zDtmfData->lastTime.tv_nsec);
						sprintf(tmpBuf2, "%.*s", 3, tmpBuf1);
						zDtmfData->lastTime.tv_nsec = atoi(tmpBuf2);
