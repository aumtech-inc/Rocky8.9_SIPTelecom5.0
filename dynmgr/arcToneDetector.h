...
/*DDN: 20091005 Modified the code to fit in ArcSipMediaMgr*/
#if 0
#include <stdio.h>
#include <stdlib.h>
#include <sys/types.h>
...
struct dtmf_data_holder
{
	int 	x;				//Bytes to read from  buffer.  Not to exceed 160
	int		last;			//Last known result
	char	frame[N+5];		//Frame size. 160 for PCMU
	int		silence_time;	//Counter before declaring valid silence data
	int		lead_silence_triggered;
	int		trail_silence_triggered;
	int		lead_silence_processed;
	int		trail_silence_processed;
	int		lookForLeadSilence;	
	int		lookForTrailSilence;
	int		MFmode;
	int		lead_silence;
	int		trail_silence;
	struct  timespec lastTime;
	time_t	last_non_silent_time;
	char	result[256];	//Keep Storing the result here.  NOT needed in ArcMediaManager
};
...
#if 0
//0.2 works with everything but with some false detection
#define RANGE  0.2           /* any thing higher than RANGE*peak is "on" */
//#define RANGE  0.14           /* any thing higher than RANGE*peak is "on" */
#define THRESH 100.0         /* minimum level for the loudest tone */
#define FLUSH_TIME 100       /* 100 frames = 3 seconds */
#endif

#if 1
#define RANGE  gDtmfToneRange          /* any thing higher than RANGE*peak is "on" */
#define THRESH gDtmfToneThreshold      /* minimum level for the loudest tone */
#define FLUSH_TIME gDtmfToneFlushTime  /* 100 frames = 3 seconds */
#endif
...
int decode(char *data,  struct dtmf_data_holder *zDtmfData);

int copy_frame(char *inbuf, char *buf);

void dtmf_tone_to_ascii(int zCall, char *inbuf, struct dtmf_data_holder *zDtmfData, int processType);
