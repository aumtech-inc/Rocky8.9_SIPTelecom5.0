...
//if(printDTMFDebug == 1)
{
//printf("RGDEBUG:: Inside ortpstack rtp_putq: this is a duplicated packet. Don't queue it (pid=%d)\n", getpid() );fflush(stdout);
} 
#endif
...
//if(printDTMFDebug == 1)
{
//printf("RGDEBUG:: rtp_getq(): asking for too old packet for (pid=%d), (ts=%d), (oldest=%d)\n", pthread_self(), timestamp, oldest);fflush(stdout);
}
#endif
...
//if(printDTMFDebug == 1)
{
//printf("RGDEBUG:: rtp_getq(): no more interesting packets, so return for (pid=%d)\n", getpid());fflush(stdout);
}
#endif
			return(ret);
		}
