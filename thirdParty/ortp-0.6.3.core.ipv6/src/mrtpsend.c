...
int main(int argc, char *argv[])
{
	...
	for (i=0;i<channels;i++){
		...
		rtp_session_set_remote_addr(session[i],argv[2],port, NULL);
	...
	while( ((i=fread(buffer,1,packet_size,infile))>0) && (runcond) )
	{
		...
		for (k=0;k<channels;k++){
			...
			if (session_set_is_set(set,session[k])){
				...
				rtp_session_send_with_ts(session[k],buffer,i,user_ts, 0, 0);
				//rtp_session_send_with_ts(session[k],buffer,i,user_ts, 0);
