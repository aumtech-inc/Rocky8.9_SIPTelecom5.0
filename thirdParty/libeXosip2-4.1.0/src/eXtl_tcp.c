...
_tcp_tl_find_socket (struct eXosip_t *excontext, char *host, int port)
{
  ...
  OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO1, NULL, "Nothing in _tcp_tl_find_socket for %s:%i:zerocount:%d nomatch:%d\n", host, port, zerocount, nomatch));
  return -1;
}
...
_tcp_tl_connect_socket (struct eXosip_t *excontext, char *host, int port)
{
  ...
  if (res)
  {
    OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO1, NULL, "Failed _eXosip_get_addrinfo for %s:%i\n", host, port));
    return -1;
  }
  ...
  for (curinfo = addrinfo; curinfo; curinfo = curinfo->ai_next) {
    ...
    if (i >= 0) {
      ...
      OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO1, NULL, "Nothing from _tcp_tl_find_socket for %s:%i\n", host, port));
    }
  }
...
_tcp_tl_update_local_target_use_ephemeral_port (struct eXosip_t *excontext, osip_message_t * req, int ephemeral_port)
{
  ...
  while (!osip_list_eol (&req->contacts, pos)) {
	...
	OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO1, NULL, "ephemeral_port is %d; msg_is_request is %d.\n",
				ephemeral_port, MSG_IS_REQUEST (req)));


 if ( (ephemeral_port > 0) &&  (MSG_IS_REQUEST (req)) ) /* BT-33 - CI:'Via' port corrupted reply from OPTIONS and INVITE request */
 {
	/*
	 * MR 5127 - skyes - not getting disconnect / eth1 used rather than eth0
	 */
	osip_via_t *via;
	(void) osip_message_get_via(req, 0, &via);

	osip_free(via->port);
	via->port = (char *) osip_malloc (10);
	snprintf(via->port, 9, "%i", ephemeral_port);
 }
...
tcp_tl_send_message (struct eXosip_t *excontext, osip_transaction_t * tr, osip_message_t * sip, char *host, int port, int out_socket)
{
  ...
  osip_naptr_t *naptr_record = NULL;
    ...
    OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO1, NULL, "Sending message (to dest=%s:%i) out_socket=%i\n", host, port, out_socket));
  ...
  if (out_socket <= 0) {
    OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO1, NULL, "Finding open connection (to dest=%s:%i) out_socket=%i\n", host, port, out_socket));
