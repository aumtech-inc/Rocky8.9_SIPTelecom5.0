...
_eXosip_snd_message (struct eXosip_t *excontext, osip_transaction_t * tr, osip_message_t * sip, char *host, int port, int out_socket)
{
  ...
  if ( p ) 
  {
	...
	if ( con &&  con->url && con->url->host   )
	{
		...
		sprintf(strTmpNewPort, "%d", tmpPort);
		osip_free(con->url->port);
		con->url->port = strdup(strTmpNewPort);
		sprintf(con->url->port, "%d", tmpPort);
		OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO2, NULL, "conUrlPort is now=[%s, %s]\n",
							con->url->host, con->url->port));
	  }
	}
...
cb_transport_error (int type, osip_transaction_t * tr, int error)
{
  ...
  OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO1, NULL, "cb_transport_error (id=%i) (err=%d)\r\n", tr->transactionid, error));
