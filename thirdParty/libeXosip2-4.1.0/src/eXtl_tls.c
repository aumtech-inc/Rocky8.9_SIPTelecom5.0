...
eXosip_set_tls_ctx (struct eXosip_t *excontext, eXosip_tls_ctx_t * ctx)
{
...
#if 1
  sprintf(client->cert, "%s", "c.pem"); 
  sprintf(server->cert, "%s", "s.pem"); 
  /* sprintf(server->cert, "%s", "\0");  */

  sprintf(client->priv_key, "%s", "ckey.pem");
  sprintf(server->priv_key, "%s", "skey.pem");
  /* sprintf(server->priv_key, "%s", "\0"); */

  sprintf(client->priv_key_pw, "%s", "password"); 
  sprintf(server->priv_key_pw, "%s", "password"); 
  /* sprintf(server->priv_key_pw, "%s", "\0");  */
#endif
  ...
  if (server->cert[0] != '\0') {
	...
	if(ctx->dh_param[0] == 0)
		sprintf(ctx->dh_param, "%s", "dh1024.pem");

	if(ctx->random_file[0] == 0)
		sprintf(ctx->random_file, "%s", "random.pem");

	/* if(ctx->root_ca_cert[0] == 0)
		sprintf(ctx->root_ca_cert, "%s", "ca.pem"); */
...
/*  OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO3, NULL, "eXosip: DDN: dh:'%s'\n", ctx->dh_param)); */
...
initialize_client_ctx (struct eXosip_t * excontext, const char *certif_client_local_cn_name, eXosip_tls_ctx_t * client_ctx, int transport)
{
  ...
  SSL_CTX_set_options (ctx, SSL_OP_ALL | SSL_OP_NO_SSLv2 | SSL_OP_NO_TLSv1 | SSL_OP_NO_TLSv1_1 | SSL_OP_NO_SESSION_RESUMPTION_ON_RENEGOTIATION | SSL_OP_CIPHER_SERVER_PREFERENCE);
...
initialize_server_ctx (struct eXosip_t * excontext, const char *certif_local_cn_name, eXosip_tls_ctx_t * srv_ctx, int transport)
{
  ...
  /* SSL_CTX_set_options (ctx, SSL_OP_ALL | SSL_OP_NO_SSLv2 | SSL_OP_NO_SESSION_RESUMPTION_ON_RENEGOTIATION | SSL_OP_CIPHER_SERVER_PREFERENCE); */
  SSL_CTX_set_options (ctx, SSL_OP_ALL | SSL_OP_NO_SSLv2 | SSL_OP_NO_SESSION_RESUMPTION_ON_RENEGOTIATION);
  ...
  while (!osip_list_eol (&req->contacts, pos)) {
    ...
    if (co != NULL && co->url != NULL && co->url->host != NULL) {
      if (ephemeral_port > 0) {
		...
		{
        	sprintf (co->url->port, "%d", tmpPort);
		}

        OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO2, NULL,
		                  "set co->url->host/port to [%s:%s]\n",  co->url->host, co->url->port));
