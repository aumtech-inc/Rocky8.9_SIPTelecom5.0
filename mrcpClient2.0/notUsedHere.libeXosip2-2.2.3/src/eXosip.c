...
{
	int i = 0;
      OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_ERROR, NULL, "eXosip: trying to lock\n"));
  i =  osip_mutex_lock ((struct osip_mutex *) eXosip.j_mutexlock);
      OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_ERROR, NULL, "eXosip: locked\n"));
...
{
	int i = 0;
      OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_ERROR, NULL, "eXosip: trying to ulock\n"));
  i =  osip_mutex_unlock ((struct osip_mutex *) eXosip.j_mutexlock);
      OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_ERROR, NULL, "eXosip: unlocked\n"));
	return(i);
}
...
/* YYQ: UPDATE START */
/*DDN: Added 07/03/2006*/
int
eXosip_modify_authentication_info (const char *username, const char *userid,
				...
				const char *realm)
{
	...
	for (jauthinfo = eXosip.authinfos; jauthinfo != NULL;
	     jauthinfo = eXosip.authinfos->next)
	{
		if(strcmp(realm, jauthinfo->realm) == 0)
		{
			sprintf(jauthinfo->username, username);
			sprintf(jauthinfo->userid, userid);
			sprintf(jauthinfo->passwd, passwd);
			sprintf(jauthinfo->ha1, ha1);
		}
	}

	return 0;

}/*END: int eXosip_modify_authentication_info*/
