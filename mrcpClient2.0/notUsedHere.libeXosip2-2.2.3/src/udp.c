...
eXosip_process_notify_for_refer (eXosip_call_t * jc, eXosip_dialog_t * jd,
                                 ...
                                 osip_event_t * evt)
{
/* YYQ: unsure if need comment next line to match ARC version   */
#if 0
...
/* YYQ   UPDATE START  */
  /* SSX START */
		...
		je = eXosip_event_init_for_call(EXOSIP_SUBSCRIPTION_NOTIFY, 
                  jc, jd, transaction);

        if (je!=NULL)
        {

            if(evt && evt->sip && evt->sip->bodies)
            {
                int yIntListSize = osip_list_size (evt->sip->bodies);

                osip_body_t * myTransferCallResult = 
                            (osip_body_t *)osip_list_get (evt->sip->bodies, 0);

                if(myTransferCallResult != NULL)
                {
                    eXosip_event_add_transferCallResult(je, 
                               myTransferCallResult->body);
...
/* YYQ update START */
#if 0
  report_call_event (EXOSIP_CALL_ACK, jc, jd, NULL);
  osip_event_free (evt);
#endif
/* YYQ update END */
...
/* YYQ: UPDATE START  */ 
  ...
  */
  /*SSX START*/
 	int ssxrc = 0;
 	ssxrc = osip_transaction_set_your_instance (transaction, __eXosip_new_jinfo (jc, jd, NULL, NULL));
	if(ssxrc != 0)
	{
		osip_free(answer);
		return NULL;
...
/* YYQ: UPDATE START (update??? YYQ: should not update to ARC version)  */ 
  report_call_event (EXOSIP_CALL_REINVITE, jc, jd, transaction);
  /* ARC's version: confirmed with Deep to change to EXOSIP_CALL_REINVITE
  report_call_event (EXOSIP_CALL_MODIFIED, jc, jd, transaction);
  */
/* YYQ: UPDATE END    */ 
...
/*  YYQ: UPDATE START  */
      ...
      /* old name in 1-9-0 version
      report_call_event (EXOSIP_CALL_NEW, jc, jd, transaction);
      */
/*  YYQ: UPDATE END  */
    }
...
/*SSX START*/
	  	//osip_transaction_set_your_instance (transaction, NULL);
		ssxrc = osip_transaction_set_your_instance (transaction, NULL);
		if(ssxrc != 0)
		{
			osip_event_free(evt);
	  	...
	  	//osip_transaction_add_event (transaction, evt);
		ssxrc = osip_transaction_add_event (transaction, evt);
		if(ssxrc != 0)
		{
			osip_event_free(evt);
            osip_transaction_free (transaction);
...
/* YYQ: UPDATE START  */
  //if (tr != NULL && tr->state != IST_TERMINATED && tr->state != ICT_TERMINATED && tr->birth_time + 180 < now) 
