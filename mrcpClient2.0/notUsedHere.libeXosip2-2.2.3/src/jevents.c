...
int
eXosip_event_add_transferCallResult(eXosip_event_t *je, char *zTransferResult)
{
    if(!je)
    {
        return (-1);
    ...
    sprintf(je->transferResult, "%s", zTransferResult);

    je->notifyType = ARCSIP_NOTIFY_TYPE_CISCO_TRANSFER_RESULT;
...
/* YYQ: UPDATE START (not updated yet, not sure  */
  } else if (type == EXOSIP_CALL_INVITE)
  /*} else if (type == EXOSIP_CALL_NEW) */ /*ARC version or 1.9.0 version ?  */
/* YYQ: UPDATE END */
    {
...
/* test: fails - should not update */
/*
 } else if (type == EXOSIP_CALL_MODIFIED)
    {
      sprintf ((*je)->textinfo, "Call parameter might be modified!");
  } else if (type == EXOSIP_CALL_REFERED)
    {
      sprintf ((*je)->textinfo, "Call transfer asked!");
  } else if (type == EXOSIP_CALL_REFER_STATUS)
    {
      sprintf ((*je)->textinfo, "New status for call transfer receveid!");
      fflush(stdout);
*/
/*YYQ: UPDATE END */
...
/* YYQ: update START */
	...
	int rc = 0;
      ...
      if (FD_ISSET (jpipe_get_read_descr(eXosip.j_socketctl_event), &fdset))
	  {
		  ...
		  rc = jpipe_read (eXosip.j_socketctl_event, buf, 1);
      //printf("DDNDEBUG: %s %s %d Read(%d) bytes BEFORE tryget\n", 
      //     __FILE__, __FUNCTION__, __LINE__, rc);fflush(stdout);
	  }
      je = (eXosip_event_t *) osip_fifo_tryget(eXosip.j_events);
      if (je!=NULL) 
	  {
      //printf("DDNDEBUG: %s %s %d Returning without reading pipe, jevent was not null\n", 
      //__FILE__, __FUNCTION__, __LINE__);fflush(stdout);
	  	return je;
	  }
	else if(rc > 0)
	{
    //printf("DDNDEBUG: %s %s %d Read(%d) bytes BEFORE tryget, but je was null\n", 
    //__FILE__, __FUNCTION__, __LINE__, rc);fflush(stdout);
	}
	/*END: DDN ADDED*/
/* YYQ: update END */
