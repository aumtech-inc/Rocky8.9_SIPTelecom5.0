...
/* YYQ: update START -- no update indeed  */
  char localip[256];

  memset (&eXosip, 0, sizeof (eXosip));
 /* memset (eXosip.localport, '\0', sizeof (eXosip.localport)); */ /* Deep's version. compilation fail  */
...
/* YYQ: update START  */
/* unsure if needed: not in 1_9_1 version, in Deep's version only */

/* testing: compilation fails:  'eXosip_t' has no member named 'ip_family' */
/*
	if (ipv6_enable == 0)
	...
	eXosip_guess_localip (eXosip.ip_family, localip, sizeof (localip));
	if (localip[0] == '\0')
	{
		OSIP_TRACE (osip_trace
			    (__FILE__, __LINE__, OSIP_ERROR, NULL,
  ...
  /*if (lower_tv.tv_sec > 15) */
    {
      ...
      OSIP_TRACE (osip_trace
                  (__FILE__, __LINE__, OSIP_INFO2, NULL,
                   "eXosip: Reseting timer to 2s before waking up!\n"));
