...
Update: 09/19/2014 djb MR 4372; adjust parsing to return the best, not last
------------------------------------------------------------------------------*/
...
ParserGlobals yParserGlobals[960];

XmlResultInfo	bestResults[960];
int				updateBestResults[960];
float			confidenceBestResults[960];
...
int executeInputTag(int zPort, DOM_Node aNode)
{
	...
	if ( updateBestResults[zPort] )
	{
		sprintf( bestResults[zPort].inputValue, "%s", nodeNameChar);
//		fprintf(stderr, "[%s, %d] save bestResults[%d].inputValue=(%s)\n",
//				__FILE__, __LINE__, zPort, bestResults[zPort].inputValue);
	}
					...
					if ( attribute != NULL )
					{
						if(attribute.getNodeName().equals("confidence"))
						{
							...
							if(nodeNameChar != NULL)
							{
								...
								if ( updateBestResults[zPort] )
								{
									bestResults[zPort].confidence = 
										yParserGlobals[zPort].gResultPtr->confidence;
//									fprintf(stderr, "[%s, %d] save bestResults[%d].confidence=(%s)\n",
//												__FILE__, __LINE__, zPort, bestResults[zPort].confidence);
								}
				...
				if(nodeNameChar != NULL)
				{
					...
					if ( updateBestResults[zPort] )
					{
						sprintf( bestResults[zPort].inputMode, "%s", nodeNameChar);
//						fprintf(stderr, "[%s, %d] save bestResults[%d].inputMode=(%s)\n",
//								__FILE__, __LINE__, zPort, bestResults[zPort].inputMode);
					}
		...
		for (int i = 0; i < attrCount; i++)
   		{
			...
			if (attribute.getNodeName().equals("confidence"))
			{
				...
				if(nodeNameChar != NULL)
				{
					float tmpF = atof(nodeNameChar);

					if ( tmpF > confidenceBestResults[zPort] )
					{
						confidenceBestResults[zPort] = tmpF;
						updateBestResults[zPort] = 1;
//						fprintf(stderr, "[%s, %d] set updateBestResults[%d]=%d\n",
//								__FILE__, __LINE__, zPort, updateBestResults[zPort]);
					}
					else
					{
						updateBestResults[zPort] = 0;
//						fprintf(stderr, "[%s, %d] set updateBestResults[%d]=%d\n",
//								__FILE__, __LINE__, zPort, updateBestResults[zPort]);
					}
			...
			if(attribute.getNodeName().equals("grammar"))
			{
				...
				if(nodeNameChar != NULL)
				{
					...
					if ( updateBestResults[zPort] )
					{
						sprintf( bestResults[zPort].resultGrammar, "%s", nodeNameChar);
					}
					delete(nodeNameChar);
					nodeNameChar = NULL;
				}
...
//			fprintf(stderr, "[%s, %d] setting instance=(%s); updateBestResults[%d]=%d\n",
//								__FILE__, __LINE__, yParserGlobals[zPort].gResultPtr->instance,
//								zPort, updateBestResults[zPort]);
			if ( updateBestResults[zPort] )
			{
				sprintf(bestResults[zPort].instance, "%s", NodeValue);
//				fprintf(stderr, "[%s, %d] save bestResults[%d].instance=(%s)\n",
//						__FILE__, __LINE__, zPort, bestResults[zPort].instance);
			}
			continue;
		}
				...
				XmlResultInfo* lpResult, char*zErrMsg)
{
	...
	static pthread_mutex_t	memBufLock;
	static int				memBufLockInitialized = 0;

	static	char  ddnTest[1234] = "DDN_TEST_STRING";

	if(memBufLockInitialized == 0)
	{
		pthread_mutex_init(&memBufLock, NULL);
		memBufLockInitialized = 1;
	}
...
//	memset((XmlResultInfo *) lpResult, '\0', sizeof(XmlResultInfo));
	...
	bestResults[zPort].confidence = 0;
	bestResults[zPort].resultGrammar[0] = '\0';
	bestResults[zPort].inputValue[0] = '\0';
	bestResults[zPort].instance[0] = '\0';
	bestResults[zPort].inputMode[0] = '\0';
	updateBestResults[zPort] = 0;
	confidenceBestResults[zPort] = 0.0;
	...
	if(platformInitDone == 2)	/*First call is creating the xml platform*/
	{
		sleep(1);
	}

	pthread_mutex_lock(&memBufLock);
	MemBufInputSource *lpMemBuf = 
		new  MemBufInputSource((const XMLByte*)pData, pSize, (char*)NULL, false);
	pthread_mutex_unlock(&memBufLock);
    ...
    if (!errorsOccured && !errReporter->getSawErrors())
    {
		...
		if(rc == 0)
		{
			...
			{
				if ( bestResults[zPort].resultGrammar[0] != '\0' )
				{
					sprintf(yParserGlobals[zPort].gResultPtr->resultGrammar, 	"%s",
								 bestResults[zPort].resultGrammar);
				}
				else
				{
					sprintf(yParserGlobals[zPort].gResultPtr->resultGrammar, 	"%s",
			...
			if ( bestResults[zPort].inputValue[0] != '\0' )
			{
				sprintf(lpResult->inputValue,  "%s", bestResults[zPort].inputValue); 
			}
			else
			{
			...
			if ( bestResults[zPort].instance[0] != '\0' )
			{
				sprintf(lpResult->instance,  "%s", bestResults[zPort].instance); 
			}
			else
			{
				if(yParserGlobals[zPort].gReturnValue[0] != '\0')
				{
					sprintf(lpResult->instance,  "%s", yParserGlobals[zPort].gResultPtr->instance);
				}
			}
			    ...
			    (strcmp(yParserGlobals[zPort].gResultPtr->inputMode, "DTMF") == 0) )
			{
				if(yParserGlobals[zPort].gDtmfValue[0])
				{
					...
					{
						if ( bestResults[zPort].confidence > 0 )
						{
							lpResult->confidence = bestResults[zPort].confidence;
						}
						else
						{
...
//		sprintf(lpResult->interpretationGrammar, 	"%s", "\0");
    ...
    if ( (p = (char *)strchr((char *)lpResult->resultGrammar, '#')) !=
                                    (char *)NULL)
    {
        *p = '\0';
    }
